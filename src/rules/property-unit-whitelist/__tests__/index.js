...
test("deprecation warning", t => {
  const config = {
    rules: {
      "property-unit-whitelist": { "font-size": ["px"] },
    },
  }
  let planned = 0

  postcss().use(stylelint(config)).process("a { font-size: 10em; }").then(result => {
    t.equal(result.warnings().length, 2)
    t.equal(result.warnings()[0].text, "'property-unit-whitelist' has been deprecated, and will be removed in '7.0'. Use 'declaration-property-unit-whitelist' instead.")
    t.equal(result.warnings()[0].stylelintType, "deprecation")
    t.equal(result.warnings()[1].text, "Unexpected unit \"em\" for property \"font-size\" (property-unit-whitelist)")
  }).catch(logError)
  planned += 4
