...
test("deprecation warning", t => {
  ...
  postcss().use(stylelint(config)).process("a { font-size: 10px; }").then(result => {
    t.equal(result.warnings().length, 2)
    t.equal(result.warnings()[0].text, "'property-unit-blacklist' has been deprecated, and will be removed in '7.0'. Use 'declaration-property-unit-blacklist' instead.")
    t.equal(result.warnings()[0].stylelintType, "deprecation")
    t.equal(result.warnings()[1].text, "Unexpected unit \"px\" for property \"font-size\" (property-unit-blacklist)")
  }).catch(logError)
  planned += 4
