...
export default function (space, options = {}) {
  ...
  return (root, result) => {
    ...
    function indentationLevel(node, level = 0) {
      ...
      if (
        optionsHasKeyword(options, "except", "block")
    ...
    function checkValue(decl, declLevel) {
      ...
      if (optionsHasKeyword(options, "ignore", "value")) { return }

      const declString = decl.toString()
      const valueLevel = (optionsHasKeyword(options, "except", "value"))
        ? declLevel
        : declLevel + 1
    ...
    function checkAtRuleParams(atRule, ruleLevel) {
      if (optionsHasKeyword(options, "ignore", "param")) { return }

      // @nest rules should be treated like regular rules, not expected
      // to have their params (selectors) indented
      const paramLevel = (optionsHasKeyword(options, "except", "param") || atRule.name === "nest")
        ? ruleLevel
        : ruleLevel + 1
    ...
    function checkMultilineBit(source, newlineIndentLevel, node) {
      ...
      styleSearch({
        source,
        target: "\n",
        outsideParens: optionsHasKeyword(options, "ignore", "inside-parens"),
      }, (match, matchCount) => {
        const precedesClosingParenthesis = /^[ \t]*\)/.test(source.slice(match.startIndex + 1))

        if (
          optionsHasKeyword(options, "ignore", "inside-parens")
          && (precedesClosingParenthesis || match.insideParens)
        ) { return }

        let expectedIndentLevel = newlineIndentLevel
        // Modififications for parenthetical content
        if (!optionsHasKeyword(options, "ignore", "inside-parens") && match.insideParens) {
