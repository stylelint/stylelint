...
declare namespace stylelint {
	...
	type Message<T, method extends string, types extends Record<string, unknown[]>> = {
		[key in `${method}`]: (
			...args: T extends keyof types ? types[T] : T extends unknown[] ? T : never
		) => string;
	};

	type ExpectedMessage<T> = Message<
		T,
		'expected',
		{
			autofix: [actual: string, expected: string];
			pattern: [input: string, pattern: StringOrRegex];
			max: [object: string, maximum: number];
		}
	>;

	type RejectedMessage<T> = Message<T, 'rejected', {}>;
	...
	>;
	type PatternRule<S extends object = {}> = CoreRule<StringOrRegex, S, ExpectedMessage<'pattern'>>;
	type MaxRule<S extends object = {}> = CoreRule<number, S, ExpectedMessage<'max'>>;
	...
	export type CoreRules = {
		...
		'color-function-notation': NotationRule<
			'modern' | 'legacy',
		...
		>;
		'color-hex-length': CoreRule<'short' | 'long', {}, ExpectedMessage<'autofix'>>;
		...
		'declaration-property-max-values': CoreRule<Record<string, number>, {}, ExpectedMessage<'max'>>;
		...
		'function-url-scheme-disallowed-list': CoreRule<
			...
			RejectedMessage<[scheme: string]>
		>;
		'hue-degree-notation': CoreRule<'angle' | 'number', {}, ExpectedMessage<'autofix'>>;
		'import-notation': CoreRule<'string' | 'url', {}, ExpectedMessage<'autofix'>>;
		...
		>;
		'lightness-notation': CoreRule<'percentage' | 'number', {}, ExpectedMessage<'autofix'>>;
		...
		'media-feature-name-unit-allowed-list': CoreRule<
			Record<string, OneOrMany<string>>,
			{},
			// FIXME should be [name: string, unit: string]
		...
		'selector-type-no-unknown': CoreRule<
			...
			RejectedMessage<[selector: string]>
		>;
		'shorthand-property-no-redundant-values': CoreRule<true, {}, ExpectedMessage<'autofix'>>;
