...
Starting from 16.0.0, the CLI prints problems to stderr instead of stdout.
...
## Changed Node.js API behavior

The Promise returned by `stylelint.lint()` has the following changes:

- The [output](../user-guide/node-api.md#output) property of the resolved object now only contains formatted problems.
- A new [code](../user-guide/node-api.md#code-1) property which contains the autofixed code has been added to the resolved object.
