...
We've added a `languageOptions.nesting` configuration property that lets you switch between standard and nonstandard nesting resolution in a number of rules, including:
...
Modern standard nesting differs from the nesting found in earlier editor drafts of the CSS specification and in language extensions like SCSS; e.g., it desugars to `:is()` and doesn't concatenate strings.
...
Is resolved to the following before the rules make their checks:
...
We've turned on standard nesting in the latest version of our [shared config](https://www.npmjs.com/package/stylelint-config-standard) as it's tailored towards helping you write spec-compliant CSS. If you don't extend our shared config, you can opt into it yourself with:
...
If you author in a language extension that uses nonstandard nesting, like SCSS, we encourage you to explicitly set the mode to `nonstandard` so that you're ready for the next major release, when we'll make standard nesting the default in Stylelint.
