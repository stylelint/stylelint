...
We've changed `no-duplicate-selector` and `selector-no-qualifying-type` to resolve selectors according to [standard nesting](https://drafts.csswg.org/css-nesting), rather than nonstandard nesting of language extensions such as SCSS, before checking them.
...
You should turn the `*-specificity`, `no-duplicate-selector` and `selector-no-qualifying-type` rules off in your config if you use a language extension that uses nonstandard nesting, such as SCSS or Less.
...
This behaviour, when hidden, can lead to surprising lint problems being flagged. With no longer desugaring nesting in the syntax rules, the selectors you see in your source files are now linted as-is, so there are no surprises.
