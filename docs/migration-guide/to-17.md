...
- deprecated the `resolveNestedSelectors` option of `selector-class-pattern`

### Deprecated the `resolveNestedSelectors` option of `selector-class-pattern`

We've deprecated the option because standard CSS nesting doesn't concatenate strings. We'll remove the option in the next major release.

You can use the new `nesting` `languageOptions` property in your configuration object instead:
