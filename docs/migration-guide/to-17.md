...
- support for Node.js less than 20.19.0
- GitHub formatter
...
You should update your config to use full paths for locators, such as `extends` and `plugins`:
...
And when using our [Jest preset](https://www.npmjs.com/package/jest-preset-stylelint) to test your plugin(s):
...
We also recommend updating your config, plugin(s), and any consuming code to ESM (ECMAScript modules) as detailed in our [16.0.0 migration guide](./to-16.md#deprecated-commonjs-api).
...
The SCSS community has turned off the `no-descending-specificity` rule in the latest version of [their shared config](https://www.npmjs.com/package/stylelint-config-standard-scss).
...
The SCSS community has turned off the `no-duplicate-selectors` rule in the latest version of [their shared config](https://www.npmjs.com/package/stylelint-config-standard-scss).
