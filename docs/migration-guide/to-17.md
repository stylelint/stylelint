...
We've changed the following `*-list` rules so that their treatment of vendor prefixes is consistent with the other `*-list` rules and with the `ignore*: []` secondary options:
...
These rules silently allowed or disallowed vendor-prefixed versions of the specified constructs. For example:

```json
{ "selector-pseudo-class-allowed-list": ["hover"] }
```

Would problematically allow nonexistent pseudo-classes such as: `:-webkit-hover`.

If you want to (dis)allow prefixes before constructs, you can use a regular expression. For example:

```diff js
-{ "at-rule-allowed-list": ["keyframes"] }
+{ "at-rule-allowed-list": ["/^(-webkit-|-moz-)?keyframes$/"] }
```
