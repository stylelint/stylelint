...
This behaviour, when hidden, can lead to surprising lint problems being flagged. As we no longer desugaring nesting in the `selector-max-*` syntax rules, the selectors you see in your source files are now linted as-is, so there are no surprises.
