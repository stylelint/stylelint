...
function finalizeResolution(resolved, base, preserveSymlinks) {
  ...
  if (!preserveSymlinks) {
    ...
    resolved = node_url.pathToFileURL(real + (filePath.endsWith(path2.sep) ? "/" : ""));
...
function defaultResolve(specifier, context = {}) {
  ...
  if (parentURL) {
    try {
      parsedParentURL = new node_url.URL(parentURL);
    } catch {
    }
  }
  let parsed;
  try {
    parsed = shouldBeTreatedAsRelativeOrAbsolutePath(specifier) ? new node_url.URL(specifier, parsedParentURL) : new node_url.URL(specifier);
  ...
  const url = moduleResolve(specifier, new node_url.URL(parentURL), conditions, false);
  return {
