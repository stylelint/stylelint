...
const importLazy = require("import-lazy")((moduleId /*: string*/) => {
  try {
    return dynamicRequire(moduleId);
  } catch (ex) {
    const error /*: {
      module?: string,
    } & Error */ = new Error(
      `Cannot resolve PostCSS syntax module "${moduleId}", please install it and try again`
    );
