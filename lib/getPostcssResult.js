...
module.exports = function (stylelint, options = {}) {
	...
	return getCode
		.then((code) => {
			const { customSyntax: customSyntaxOption, syntax: syntaxOption } = stylelint._options;

			/** @type {Syntax | null} */
			let syntax = null;

			if (customSyntaxOption) {
				try {
					// TODO TYPES determine which type has customSyntax
					const customSyntax = /** @type {any} */ require(customSyntaxOption);
					...
					throw new Error(`Cannot resolve custom syntax module ${customSyntaxOption}`);
				}
			} else if (syntaxOption === 'css') {
				syntax = cssSyntax(stylelint);
			} else if (syntaxOption) {
				const keys = Object.keys(syntaxes);

				if (!keys.includes(syntaxOption)) {
					throw new Error(
