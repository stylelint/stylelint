...
module.exports = function (stylelint/*: stylelint$internalApi*/)/*: Promise<?Object>*/ {
  ...
  const cached/*: Object*/ = stylelint._postcssResultCache.get(options.filePath)
  ...
  return getCode.then(code => {
    ...
    if (customSyntax) {
      ...
      const fileExtension/*: string*/ = path.extname(options.filePath || "")
    ...
    const source/*: ?string*/ = options.code ? options.codeFilename : options.filePath
    let preProcessedCode/*: string*/ = code
