...
module.exports = function(stylelint) {
	...
	return getCode
		.then((code) => {
			...
			/** @type {{parse: any, stringify: any}} */
			let syntax = /** @type {any} */ (stylelint._options.syntax);

			if (customSyntax) {
				try {
					// TODO TYPES determine which type has customSyntax
					syntax = /** @type {any} */ dynamicRequire(customSyntax);
				} catch (e) {
				...
				if (!stylelint._options.syntax) {
					throw new Error(UNKNOWN_SYNTAX_ERROR);
				}
		...
		.then((postcssResult) => {
			if (options.filePath) {
				// TODO TYPES determine which type of result is here? with stylelint result or without?
				stylelint._postcssResultCache.set(
					options.filePath,
					/** @type {import('stylelint').PostcssResult} */ (postcssResult),
				);
			}
...
/**
 * @param {import('stylelint').StylelintInternalApi} stylelint
 * @return {{parse: any, stringify: any}}
 */
