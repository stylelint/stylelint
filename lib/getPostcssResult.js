...
module.exports = function (stylelint, options = {}) {
	...
	return getCode
		.then((code) => {
			return stylelint
				.getConfigForFile(stylelint._options.configFile || options.filePath)
				.then((config) => ({
					// config: (config && config.config) || {},
					config: { ...((config && config.config) || {}), ...stylelint._options.config },
					code,
				}));
		})
		.then(({ code, config }) => {
			...
			const cssInJsConfig = {
				unstable_substitute: config.unstable_substituteCssInJs,
			};
			...
			if (stylelint._options.customSyntax) {
				...
				if (stylelint._options.syntax === 'css') {
					...
					const syntaxKey = stylelint._options.syntax;

					if (!isSyntaxKey(syntaxKey)) {
						throw new Error(
					...
					syntax =
						syntaxKey === 'css-in-js' ? syntaxes['css-in-js'](cssInJsConfig) : syntaxes[syntaxKey];
				}
				...
				const { html, 'css-in-js': cssInJs, ...rest } = syntaxes;

				syntax = autoSyntax({
					css: cssSyntax(stylelint),
					jsx: cssInJs(cssInJsConfig),
					...rest,
				});
			}
 ...
 *
 * @param {string} key
 * @returns {key is keyof typeof syntaxes}
 */
function isSyntaxKey(key) {
	return Object.keys(syntaxes).includes(key);
}
