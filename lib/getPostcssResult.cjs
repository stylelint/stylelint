...
const require$1 = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (document.currentScript && document.currentScript.src || new URL('lib/getPostcssResult.cjs', document.baseURI).href)));
 ...
 */
async function getPostcssResult(stylelint, options = {}) {
	...
	if (options.code !== undefined) {
		...
		getCode = await promises.readFile(options.filePath, 'utf8');
	}
...
function getCustomSyntax(customSyntax, basedir) {
	if (typeof customSyntax === 'string') {
		...
		try {
			resolved = require$1(customSyntaxLookup);
		} catch (error) {
			if (
				error &&
				typeof error === 'object' &&
...
function cssSyntax(stylelint, filePath) {
	const fileExtension = filePath ? node_path.extname(filePath).slice(1).toLowerCase() : '';
