...
export default async function getPostcssResult(stylelint, { customSyntax, filePath, code } = {}) {
	...
	const fix = normalizeFixMode(stylelint._options, stylelint._options.config);
...
/**
 * @param {import('stylelint').Config['fix']} fix
 * @returns {Promise<Syntax>}
 */
async function cssSyntax(fix) {
	const parse = await (fix === 'lax'
		? import('postcss-safe-parser').then((m) => m.default)
		: postcss.parse);
