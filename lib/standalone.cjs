...
const createReturnedResults = require('./createReturnedResults.cjs');
...
async function standalone({
	...
	if (!files) {
		...
		try {
			...
			stylelintResult = await createReturnedResults(stylelint, postcssResult, absoluteCodeFilename);
		} catch (error) {
	...
	if (filePaths.length) {
		...
		const getStylelintResults = absoluteFilePaths.map(async (absoluteFilepath) => {
			...
			try {
				...
				return await createReturnedResults(stylelint, postcssResult, absoluteFilepath);
			} catch (error) {
...
async function handleError(stylelint, error, filePath = undefined) {
	if (error instanceof Error && error.name === 'CssSyntaxError') {
		return await createReturnedResults(
			stylelint,
			undefined,
			filePath,
