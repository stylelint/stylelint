...
const emitExperimentalWarning = require('./utils/emitExperimentalWarning.cjs');
...
async function standalone({
	...
	const useStdin = !files && validateTypes.isString(code);
	const hasOneValidInput = (files && !validateTypes.isString(code)) || useStdin;
	...
	if (!useStdin) {
		...
		if (suppressAll || suppressRule || suppressPrune || fs.existsSync(suppressionsFileLocation)) {
			emitExperimentalWarning.default(
	...
	if (unusedSuppressionsCount > 0 && !suppressPrune) {
		process.stderr.write(
			...
			)} Consider re-running the command with ${dim('--suppress-prune')}.${node_os.EOL}`,
		);

		process.exitCode = constants.EXIT_CODE_LINT_PROBLEM;
	}
