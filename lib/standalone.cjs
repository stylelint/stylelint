...
const getCacheFile = require('./utils/getCacheFile.cjs');
...
async function standalone({
	...
	if (!useStdin) {
		...
		if (suppressAll || suppressRule || pruneSuppressions || fs.existsSync(suppressionsFileLocation)) {
	...
	if (unusedSuppressionsCount > 0 && !pruneSuppressions) {
		process.stderr.write(
			...
			)} Consider re-running the command with ${dim('--prune-suppressions')}.${node_os.EOL}`,
		);
