...
export default function needlessDisables(postcssResult) {
	...
	const { disabledRanges, disabledWarnings = [] } = postcssResult.stylelint;
	const disabledRangesAll = disabledRanges[RULE_NAME_ALL] ?? [];
	...
	const allRangeComments = new Set(disabledRangesAll.map((range) => range.comment));

	for (const [rule, ranges] of Object.entries(disabledRanges)) {
		for (const range of ranges) {
			const commentNode = range.comment;

			if (rule !== RULE_NAME_ALL && allRangeComments.has(commentNode)) continue;
