...
const require$1 = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (document.currentScript && document.currentScript.src || new URL('lib/augmentConfig.cjs', document.baseURI).href)));
...
function addPluginFunctions(config) {
	...
	for (const pluginLookup of normalizedPlugins) {
		...
		if (typeof pluginLookup === 'string') {
			pluginImport = require$1(pluginLookup);
		} else {
