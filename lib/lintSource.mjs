...
export default async function lintSource(stylelint, options = {}) {
	...
	const lexer = shouldUseLexer(config.rules)
		? fork({
 ...
 * @returns {boolean}
 */
function shouldUseLexer(rules) {
	return rulesUsingLexer.some((rule) => rules[rule] !== undefined);
}
