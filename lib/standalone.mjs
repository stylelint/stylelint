...
export default async function standalone({
	...
	const postProcessorFunction = await getPostProcessorFunction(stylelint);
	...
	if (!files) {
		...
		if (
			...
			return prepareReturnValue([], maxWarnings, formatterFunction, postProcessorFunction, cwd);
		}
...
/**
 * @param {import('stylelint').InternalApi} stylelint
 * @returns {Promise<import('stylelint').PostProcessorFunction>}
 */
async function getPostProcessorFunction(stylelint) {
	const config = await getConfigForFile(stylelint);

	const postProcessor = config?.config?.postProcessor;
	...
	if (typeof postProcessor === 'string') {
		const basedir = stylelint._options.configBasedir;
		const postProcessorLookup = basedir ? getModulePath(basedir, postProcessor) : postProcessor;

		let resolved;

		try {
			resolved = await dynamicImport(postProcessorLookup);
			...
			if (
				...
				error.message.includes(postProcessor)
			) {
				throw new Error(
					`Cannot resolve postProcessor module "${postProcessor}". Check that module "${postProcessor}" is available and spelled correctly.\n\nCaused by: ${error}`,
				);
			}
	...
	throw new Error('Post processor must be a string or a function');
}
