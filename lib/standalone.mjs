...
/** @import {InternalApi, LintResult, SuppressedViolations} from 'stylelint' */
...
export default async function standalone({
	...
	let unusedSuppressions = new Map();
	...
	if (!useInputCode) {
		...
		if (suppressAll || suppressRule || suppressPrune || existsSuppressionsFile) {
	...
	if (!suppressPrune && unusedSuppressions.size > 0) {
