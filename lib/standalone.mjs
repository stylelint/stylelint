...
import { EXIT_CODE_LINT_PROBLEM } from './constants.mjs';
...
import emitExperimentalWarning from './utils/emitExperimentalWarning.mjs';
...
export default async function standalone({
	...
	const useStdin = !files && isString(code);
	const hasOneValidInput = (files && !isString(code)) || useStdin;
	...
	if (!useStdin) {
		const suppressionsFileLocation = resolveFilePath(
			suppressLocation || 'stylelint-suppressions.json',
			cwd,
		...
		if (suppressAll || suppressRule || suppressPrune || existsSync(suppressionsFileLocation)) {
			...
			if (suppressPrune) {
				await suppressions.prune(stylelintResults);
			}
	...
	if (unusedSuppressionsCount > 0 && !suppressPrune) {
		process.stderr.write(
			`${red(
				'There are suppressions left that do not occur anymore.',
			)} Consider re-running the command with ${dim('--suppress-prune')}.${EOL}`,
		);
