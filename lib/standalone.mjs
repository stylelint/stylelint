...
import getCacheFile from './utils/getCacheFile.mjs';
...
export default async function standalone({
	...
	if (!useStdin) {
		const suppressionsFileLocation = getCacheFile(
			suppressionsLocation || 'stylelint-suppressions.json',
			cwd,
		);
		...
		if (suppressAll || suppressRule || pruneSuppressions || existsSync(suppressionsFileLocation)) {
	...
	if (unusedSuppressionsCount > 0 && !pruneSuppressions) {
		process.stderr.write(
			`${red(
				'There are suppressions left that do not occur anymore.',
			)} Consider re-running the command with ${dim('--prune-suppressions')}.${EOL}`,
		);
