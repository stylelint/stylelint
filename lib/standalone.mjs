		...
		const getStylelintResults = absoluteFilePaths.map(async (absoluteFilepath) => {
			...
			try {
				...
				if (postcssResult.root && postcssResult.opts && !postcssResult.stylelint.ignored && fix) {
					...
					const input = postcssResult.root.source?.input;

					// `input.css` holds the source without its byte order mark, so the source as read
					// is rebuilt from the mark recorded before any fix could change `input.hasBOM`
					const sourceCss =
						input && `${postcssResult.stylelint.hasBOM ? '\uFEFF' : ''}${input.css}`;

					if (sourceCss !== undefined && sourceCss !== fixedCss) {
