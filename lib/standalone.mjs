...
import { normalizePathUrl } from './utils/normalizePathUrl.mjs';
...
export default async function standalone({
	...
	let fileList = [files].flat().map((entry) => {
		const globCWD = normalizePathUrl((globbyOptions && globbyOptions.cwd) || cwd);
	...
	if (!disableDefaultIgnores) {
		fileList = fileList.concat(ALWAYS_IGNORED_GLOBS.map((fileEl) => `!${fileEl}`));
	}
	...
	const effectiveGlobbyOptions = {
		...(globbyOptions || {}),
		cwd: normalizePathUrl((globbyOptions && globbyOptions.cwd) || cwd),
		absolute: true,
	};

	const globCWD = effectiveGlobbyOptions.cwd;

	let filePaths = await glob(fileList, effectiveGlobbyOptions);
