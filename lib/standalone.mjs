...
import getLintOptionOrConfigValue from './utils/getLintOptionOrConfigValue.mjs';
...
export default async function standalone({
	...
	const useCache = await canCache(stylelint);
	...
	} else if (await canAllowEmptyInput(stylelint)) {
...
async function canAllowEmptyInput(stylelint) {
	return getLintOptionOrConfigValue({
		stylelint,
 ...
 */
async function canCache(stylelint) {
	return getLintOptionOrConfigValue({
		stylelint,
		option: 'cache',
		config: 'cache',
