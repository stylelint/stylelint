...
export default async function standalone({
	...
	if (!files) {
		...
		if (autofix === true || autofix === 'lax' || (autofix === 'strict' && !hasErrors)) {
			if (postcssResult && !postcssResult.stylelint.ignored) {
	...
	if (filePaths.length) {
		...
		const getStylelintResults = absoluteFilePaths.map(async (absoluteFilepath) => {
			...
			try {
				...
				if (
					postcssResult.stylelint.stylelintError ||
					(postcssResult.stylelint.stylelintWarning && useCache)
				) {
