...
import createStylelintResult from './createReturnedResults.mjs';
	...
	if (filePaths.length) {
		...
		const getStylelintResults = absoluteFilePaths.map(async (absoluteFilepath) => {
			...
			try {
				...
				return await createStylelintResult(stylelint, postcssResult, absoluteFilepath);
			} catch (error) {
				// On any error, we should not cache the lint result
				stylelint._fileCache.removeEntry(absoluteFilepath);

				return await handleError(stylelint, error, absoluteFilepath);
			}
		});
 ...
 */
async function handleError(stylelint, error, filePath = undefined) {
	if (error instanceof Error && error.name === 'CssSyntaxError') {
		return await createStylelintResult(
			stylelint,
			undefined,
			filePath,
			/** @type {CssSyntaxError} */ (error),
		);
	}
