...
export default async function standalone({
	...
	const postProcessor = await getPostProcessorFunction(stylelint);
	...
	if (!files) {
		...
		if (
			...
			return prepareReturnValue([], maxWarnings, formatterFunction, postProcessor, cwd);
		}
...
/**
 * @param {import('stylelint').InternalApi} stylelint
 * @returns {Promise<import('stylelint').PostProcessor>}
 */
async function getPostProcessorFunction(stylelint) {
	const config = await getConfigForFile(stylelint);

	return config?.config?.postProcessor ?? ((result) => result);
}
