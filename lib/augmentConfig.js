...
function mergeConfigs(a, b) {
	...
	if (a.plugins || b.plugins) {
		...
		if (b.plugins) {
			pluginMerger.plugins = pluginMerger.plugins.concat(b.plugins);
		}

		pluginMerger.plugins = [...new Set(pluginMerger.plugins)];
	}
	...
	if (a.overrides || b.overrides) {
		...
		if (b.overrides) {
			overridesMerger.overrides = overridesMerger.overrides.concat(b.overrides);
		}

		// Remove duplicates, last one wins
		overridesMerger.overrides = overridesMerger.overrides.filter(
...
function applyOverrides(fullConfig, rootConfigDir, filePath) {
	...
	if (!overrides) {
		// Do mergeConfigs to remove duplicate items
