...
/** @typedef {{ config: StylelintConfig, filepath: string }} CosmiconfigResult */
 ...
 * @param {StylelintInternalApi} stylelint
 * @param {CosmiconfigResult} [cosmiconfigResultArg]
...
function loadExtendedConfig(stylelint, config, configDir, extendLookup) {
	...
	// TODO TYPES
	// fix stylelint definitions. Now it is Promise<Object | null>
	return /** @type {Promise<CosmiconfigResult | null>} */ (stylelint._extendExplorer.load(
		extendPath,
	));
}
	...
	/** @type {Array<StylelintConfigProcessors>} */ ([])
		...
		.forEach((processorConfig) => {
			...
			if (processorCache.has(processorKey)) {
				...
				/** @type {string} */
				const processorLookup = /** @type {string} */ (processorConfig[0]);
				/** @type {Object} */
