...
describe('plugin using exposed rules via stylelint.rules', () => {
	...
	const config = (expectation) => ({
		config: {
			plugins: [path.join(__dirname, 'fixtures/plugin-conditionally-check-color-named.cjs')],
			rules: {
...
describe('module providing an array of plugins', () => {
	const config = {
		plugins: [path.join(__dirname, 'fixtures/plugin-array.cjs')],
		rules: {
...
it('slashless plugin causes configuration error', async () => {
	const config = {
		plugins: [path.join(__dirname, 'fixtures/plugin-slashless-warn-about-foo.cjs')],
		rules: {
...
it('plugin with primary option array', async () => {
	const config = {
		plugins: [path.join(__dirname, 'fixtures/plugin-primary-array.cjs')],
		rules: {
...
it('plugin with async rule', async () => {
	const config = {
		plugins: [path.join(__dirname, 'fixtures/plugin-async.cjs')],
		rules: {
			'plugin/async': true,
		},
	};
