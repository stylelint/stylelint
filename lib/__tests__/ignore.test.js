...
test('extending config and ignorePatterns glob ignoring single glob', async () => {
	const { results } = await standalone({
		files: [fixtures('empty-block.css'), fixtures('invalid-hex.css')],
		config: {
			ignorePatterns: ['**/invalid-hex.css'],
...
test('same as above with no configBasedir, ignorePatterns path relative to process.cwd', async () => {
	const { results } = await standalone({
		files: [fixtures('empty-block.css'), fixtures('invalid-hex.css')],
		config: {
			ignorePatterns: ['fixtures/invalid-hex.css'],
...
test('same as above with no configBasedir, ignorePatterns path relative to options.cwd', async () => {
	const { results } = await standalone({
		files: [fixtures('empty-block.css'), fixtures('invalid-hex.css')],
		config: {
			ignorePatterns: ['fixtures/invalid-hex.css'],
...
test('absolute ignorePatterns glob path', async () => {
	const { results } = await standalone({
		files: [fixtures('empty-block.css'), fixtures('invalid-hex.css')],
		config: {
			ignorePatterns: [fixtures('empty-b*.css')],
			rules: {
				'block-no-empty': true,
			},
		},
...
test('extending config with ignorePatterns glob ignoring one by negation', async () => {
	const { results } = await standalone({
		files: [fixtures('empty-block.css'), fixtures('invalid-hex.css')],
		config: {
			ignorePatterns: ['**/*.css', '!**/invalid-hex.css'],
	...
	expect(results[0].source).toContain('invalid-hex.css');
...
test('using ignorePatterns with input files that would cause a postcss syntax error', async () => {
	const { results } = await standalone({
...
test('extending a config that ignores patterns', async () => {
	const { results } = await standalone({
		files: [fixtures('empty-block.css'), fixtures('invalid-hex.css')],
		config: {
			extends: [fixtures('config-extending-and-ignoring-patterns')],
		},
		configBasedir: fixtures(),
	});
...
test('using codeFilename and ignorePatterns together', async () => {
	const { results } = await standalone({
		code: 'a {}',
		codeFilename: replaceBackslashes(path.join(__dirname, 'foo.css')),
		config: {
			ignorePatterns: ['**/foo.css'],
...
test('using codeFilename and ignorePatterns with configBasedir', async () => {
	const { results } = await standalone({
		code: 'a {}',
		codeFilename: replaceBackslashes(path.join(__dirname, 'foo.css')),
		config: {
...
// tests for the (now-deprecated) ignoreFiles
