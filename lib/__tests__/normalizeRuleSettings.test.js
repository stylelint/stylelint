...
describe("rules whose primary option CAN BE an array", () => {
  ...
  it("nested primary option array returns same", () => {
    const actual = normalizeRuleSettings([ [ "calc", "rgba" ], { "severity": "warning" } ], "function-whitelist", true)
    const expected = [ [ "calc", "rgba" ], { "severity": "warning" } ]
    expect(actual).toEqual(expected)
  })

  it("string as first primary option returns same", () => {
    const actual = normalizeRuleSettings([ "alphabetical", { "severity": "warning" } ], "declaration-block-properties-order")
    const expected = [ "alphabetical", { "severity": "warning" } ]
    expect(actual).toEqual(expected)
  })

  it("primary option array with length of 2", () => {
    const actual = normalizeRuleSettings([ { foo: 1 }, { foo: 2 } ], "declaration-block-properties-order", true)
    const expected = [[ { foo: 1 }, { foo: 2 } ]]
    expect(actual).toEqual(expected)
  })

  it("primary option array with length of 2 and secondary options", () => {
    const actual = normalizeRuleSettings([ [ { foo: 1 }, { foo: 2 } ], { "severity": "warning" } ], "declaration-block-properties-order", true)
