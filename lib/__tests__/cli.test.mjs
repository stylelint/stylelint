...
describe('buildCLI', () => {
	...
	it('optionNames', () => {
		expect(
			buildCLI(['--fix', '-c', 'foo', '--compute-edit-info', '--foo', '-x']).optionNames,
		).toEqual(['fix', 'config', 'compute-edit-info', 'foo', 'x']);
	});

	it('flags.allow-empty-input', () => {
		expect(buildCLI(['--allow-empty-input']).flags['allow-empty-input']).toBe(true);
	});
	...
	it('flags.cache-location', () => {
		expect(buildCLI(['--cache-location=foo']).flags['cache-location']).toBe('foo');
	});

	it('flags.globby-options', () => {
		const { flags } = buildCLI(['--globby-options={"dot":true}']);

		expect(flags['globby-options']).toBe('{"dot":true}');
	});

	it('flags.cache-strategy', () => {
		expect(buildCLI(['--cache-strategy=content']).flags['cache-strategy']).toBe('content');
		expect(buildCLI(['--cache-strategy=metadata']).flags['cache-strategy']).toBe('metadata');
	});
	...
	it('flags.config-basedir', () => {
		const { flags } = buildCLI(['--config-basedir=/path/to/dir']);

		expect(flags['config-basedir']).toBe('/path/to/dir');
	});

	it('flags.custom-formatter', () => {
		expect(buildCLI(['--custom-formatter=foo']).flags['custom-formatter']).toBe('foo');
	});

	it('flags.custom-syntax', () => {
		expect(buildCLI(['--custom-syntax=foo']).flags['custom-syntax']).toBe('foo');
	});

	it('flags.disable-default-ignores', () => {
		expect(buildCLI(['--disable-default-ignores']).flags['disable-default-ignores']).toBe(true);
	});
	...
	it('flags.ignore-disables', () => {
		expect(buildCLI(['--ignore-disables']).flags['ignore-disables']).toBe(true);
	});

	it('flags.ignore-path', () => {
		const ignorePathOf = (argv) => buildCLI(argv).flags['ignore-path'];

		expect(ignorePathOf(['--ignore-path=/path/to/file'])).toEqual(['/path/to/file']);
		expect(ignorePathOf(['-i', '/path/to/file'])).toEqual(['/path/to/file']);
		expect(ignorePathOf(['--ignore-path=/path/to/file1', '--ignore-path=/path/to/file2'])).toEqual([
			'/path/to/file1',
			'/path/to/file2',
		]);
		expect(ignorePathOf(['-i', '/path/to/file1', '-i', '/path/to/file2'])).toEqual([
			'/path/to/file1',
			'/path/to/file2',
		]);
	});

	it('flags.ignore-pattern', () => {
		const ignorePatternOf = (argv) => buildCLI(argv).flags['ignore-pattern'];

		expect(ignorePatternOf(['--ignore-pattern=vendor/**'])).toEqual(['vendor/**']);
		expect(ignorePatternOf(['--ignore-pattern', 'vendor/**'])).toEqual(['vendor/**']);
		expect(
			ignorePatternOf(['--ignore-pattern', 'vendor/**', '--ignore-pattern', 'test/*.css']),
		).toEqual(['vendor/**', 'test/*.css']);
	});

	it('flags.max-warnings', () => {
		expect(buildCLI(['--max-warnings=7']).flags['max-warnings']).toBe(7);
		expect(buildCLI(['--max-warnings', '-1']).flags['max-warnings']).toBe(-1);
	});

	it('flags.output-file', () => {
		expect(buildCLI(['--output-file=/path/to/file']).flags['output-file']).toBe('/path/to/file');
		expect(buildCLI(['-o', '/path/to/file']).flags['output-file']).toBe('/path/to/file');
	});

	it('flags.print-config', () => {
		expect(buildCLI(['--print-config']).flags['print-config']).toBe(true);
	});
	...
	it('flags.quiet-deprecation-warnings', () => {
		const { flags } = buildCLI(['--quiet-deprecation-warnings']);

		expect(flags['quiet-deprecation-warnings']).toBe(true);
	});

	it('flags.report-invalid-scope-disables', () => {
		const { flags } = buildCLI(['--report-invalid-scope-disables']);

		expect(flags['report-invalid-scope-disables']).toBe(true);
	});

	it('flags.report-needless-disables', () => {
		expect(buildCLI(['--report-needless-disables']).flags['report-needless-disables']).toBe(true);
	});
	...
	it('flags.stdin-filename', () => {
		expect(buildCLI(['--stdin-filename=foo.css']).flags['stdin-filename']).toBe('foo.css');
	});
	...
	it('flags.compute-edit-info', () => {
		expect(buildCLI(['--compute-edit-info']).flags['compute-edit-info']).toBe(true);
	});
	...
	it('flags.suppress-location', () => {
		expect(buildCLI(['--suppress-location=foo']).flags['suppress-location']).toBe('foo');
	});
});
...
describe('CLI', () => {
	...
	it('--print-config', async () => {
		await cli([
			'--print-config',
			'--config',
			fixturesPath('config-relative-plugin.json'),
		...
		expect(process.stdout.write).toHaveBeenLastCalledWith(
			JSON.stringify(
				{
					plugins: [
						fileURLToPath(new URL('./fixtures/plugin-warn-about-foo.mjs', import.meta.url)),
					],
					rules: {
	...
	it('--report-needless-disables', async () => {
		...
		expect(process.stderr.write).toHaveBeenCalledWith(
			expect.stringMatching(/Needless disable for "color-named".*Empty block/s),
		);
	});
	...
	it('--no-color', async () => {
		...
		expect(process.stderr.write).toHaveBeenCalledTimes(1);
		expect(process.stderr.write).not.toHaveBeenCalledWith(expect.stringContaining('\u001B['));
	});
	...
	it('--stdin with any input from stdin', async () => {
		...
		expect(process.stdout.write).not.toHaveBeenCalled();
		expect(process.stderr.write).toHaveBeenCalledTimes(1);
		expect(process.stderr.write).toHaveBeenCalledWith(expect.stringContaining('Empty source'));
	});
	...
	it('--custom-formatter with relative path to ESM', async () => {
		await cli([
			'--custom-formatter',
			getRelativePath(process.cwd(), fixturesPath('custom-formatter.mjs')),
			'--config',
			fixturesPath('default-severity-warning.json'),
			fixturesPath('empty-block.css'),
		]);
	...
	it('--custom-formatter with relative path to CommonJS', async () => {
		await cli([
			'--custom-formatter',
			getRelativePath(process.cwd(), fixturesPath('custom-formatter.cjs')),
			'--config',
			fixturesPath('default-severity-warning.json'),
			fixturesPath('empty-block.css'),
		]);
	...
	it('output a message when unsupported --globby-options provided', async () => {
		await cli([
			'--globby-options={"cwd":"."}',
			'--config',
			fixturesPath('config-block-no-empty.json'),
		...
		expect(process.exitCode).toBe(EXIT_CODE_INVALID_CONFIG);

		expect(process.stdout.write).not.toHaveBeenCalled();
		expect(process.stderr.write).toHaveBeenCalledTimes(1);
		expect(process.stderr.write).toHaveBeenCalledWith(
			expect.stringContaining('unknown option "cwd"'),
		);
	});
	...
	it('applies --fix to given files and shows no error messages', async () => {
		...
		await fs.writeFile(cssFile1, 'a { top: 0px }');
		await fs.writeFile(cssFile2, 'b { top: 0px }');
		...
		expect(await fs.readFile(cssFile1, 'utf8')).toBe('a { top: 0 }');
	...
	it('applies --fix (strict) and fixes without error message', async () => {
		jest.spyOn(process, 'stdin', 'get').mockImplementationOnce(() =>
			Readable.from([
				Buffer.from(stripIndent`
					a { top: 0px }
	...
	it('exits with an error message when a boolean flag has a value', async () => {
		await cli(['--cache=false']);
		...
		expect(process.stderr.write).toHaveBeenCalledTimes(1);
		expect(process.stderr.write).toHaveBeenCalledWith(
			expect.stringMatching(/--cache=false.*--no-cache/),
		);
	});

	it('exits with an error message when a short flag has an = before its value', async () => {
		await cli(['-c=foo']);
		...
		expect(process.stdout.write).not.toHaveBeenCalled();
		expect(process.stderr.write).toHaveBeenCalledTimes(1);
		expect(process.stderr.write).toHaveBeenCalledWith(expect.stringMatching(/-c=foo.*-c foo/));
	});

	it('exits with an error message when a removed alias is specified', async () => {
		await cli(['--aei']);
		...
		expect(process.stderr.write).toHaveBeenCalledTimes(1);
		expect(process.stderr.write).toHaveBeenCalledWith(
			expect.stringMatching(/--aei.*--allow-empty-input/),
		);
	});
