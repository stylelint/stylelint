...
describe('buildCLI', () => {
	...
	it('flags.pruneSuppressions', () => {
		expect(buildCLI(['--pruneSuppressions']).flags.pruneSuppressions).toBe(true);
	});

	it('flags.suppressAll', () => {
		expect(buildCLI(['--suppressAll']).flags.suppressAll).toBe(true);
	});

	it('flags.suppressRule', () => {
		expect(buildCLI(['--suppressRule=foo']).flags.suppressRule).toEqual(['foo']);
		expect(buildCLI(['--suppressRule', 'foo']).flags.suppressRule).toEqual(['foo']);
		expect(buildCLI(['--suppressRule', 'foo', '--suppressRule', 'bar']).flags.suppressRule).toEqual(
			['foo', 'bar'],
		);
	});

	it('flags.suppressionsLocation', () => {
		expect(buildCLI(['--suppressionsLocation=foo']).flags.suppressionsLocation).toBe('foo');
	});
});
...
describe('CLI', () => {
	...
	describe('Suppressions', () => {
		...
		const CONFIG_BLOCK_ONLY = fixturesPath('suppressions/config-block-no-empty.json');
		...
		const INVALID_HEX_CSS = fixturesPath('suppressions/invalid-hex.css');
		...
		describe('arguments combinations', () => {
			it('displays an error when the --suppress-all and --suppress-rule flags are used together', async () => {
				await cli(['--suppress-all', '--suppress-rule', 'block-no-empty']);
				...
				expect(process.stderr.write).toHaveBeenCalledWith(
					expect.stringMatching(
						/The --suppress-all option and the --suppress-rule option cannot be used together./s,
					),
				);
			});

			it('displays an error when the --suppress-all and --prune-suppressions flags are used together', async () => {
				await cli(['--suppress-all', '--prune-suppressions']);
				...
				expect(process.stderr.write).toHaveBeenCalledWith(
					expect.stringMatching(
						/The --suppress-all option and the --prune-suppressions option cannot be used together./s,
					),
				);
			});

			it('displays an error when the --suppress-rule and --prune-suppressions flags are used together', async () => {
				await cli(['--suppress-rule', 'block-no-empty', '--prune-suppressions']);
				...
				expect(process.stderr.write).toHaveBeenCalledWith(
					expect.stringMatching(
						/The --suppress-rule option and the --prune-suppressions option cannot be used together./s,
					),
				);
			});
		});
		...
		describe('stdin', () => {
			it('displays an error when the --suppress-all flag is used', async () => {
				...
				expect(process.stderr.write).toHaveBeenCalledWith(
					expect.stringMatching(
						/The --suppress-all, --suppress-rule, and --prune-suppressions options cannot be used with piped-in code./s,
					),
				);
			});
			...
			it('displays an error when the --suppress-rule flag is used', async () => {
				...
				await cli(['--stdin', '--suppress-rule', 'block-no-empty']);
			...
			it('displays an error when the --prune-suppressions flag is used', async () => {
				...
				await cli(['--stdin', '--prune-suppressions']);
		...
		describe('when no suppression file exists', () => {
			...
			it('creates the suppressions file when the --suppress-rule flag is used, and reports some violations', async () => {
				...
				expect(process.stderr.write.mock.calls[0][0]).toMatch(/block-no-empty/);
				expect(process.stderr.write.mock.calls[0][0]).not.toMatch(/color-no-invalid-hex/);
			});
			...
			it('creates the suppressions file when multiple --suppress-rule flags are used, and reports some violations', async () => {
				...
				expect(process.stderr.write.mock.calls[0][0]).toMatch(/length-zero-no-unit/);
				expect(process.stderr.write.mock.calls[0][0]).not.toMatch(/block-no-empty/);
				expect(process.stderr.write.mock.calls[0][0]).not.toMatch(/color-no-invalid-hex/);
			});
		...
		describe('when an invalid suppressions file already exists', () => {
			...
			it('gives an error when the --prune-suppressions argument is used', async () => {
				await cli([
					'--prune-suppressions',
					'--suppressions-location',
					SUPPRESSIONS_PATH,
		...
		describe('when a valid suppressions file already exists', () => {
			...
			it('exits with code EXIT_CODE_LINT_PROBLEM, when there are unused violations', async () => {
				const sup = structuredClone(expectedSuppressions);
