...
describe('CLI', () => {
	...
	describe('--rule', () => {
		...
		it('throws if the format is invalid', async () => {
			await expect(
				cli(['--print-config', '--rule', 'color-no-invalid-hex', fixturesPath('invalid-hex.css')]),
			).rejects.toThrow("Rule's format is invalid.");
		});

		it('throws if the option is not parseable', async () => {
			await expect(
				cli(['--print-config', '--rule', 'indentation: [tab]', fixturesPath('invalid-hex.css')]),
			).rejects.toThrow("Rules' options must be JSON parseable.");
		});
