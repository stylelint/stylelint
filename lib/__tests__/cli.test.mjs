...
describe('buildCLI', () => {
	...
	it('flags.globOptions', () => {
		expect(buildCLI(['--glob-options={"dot":true}']).flags.globOptions).toBe('{"dot":true}');
		expect(buildCLI(['--go={"dot":true}']).flags.globOptions).toBe('{"dot":true}');
	});
	...
	it('flags.suppressLocation', () => {
		expect(buildCLI(['--suppress-location=foo']).flags.suppressLocation).toBe('foo');
	});
});
...
describe('CLI', () => {
	...
	it('output a message when wrong --glob-options provided', async () => {
		await cli(['--glob-options=wrong']);
		...
		expect(stripVTControlCharacters(process.stderr.write.mock.calls[0][0])).toContain(
			'Invalid option "--glob-options". The value "wrong" is not valid JSON object.',
		);
	});

	it('--glob-options', async () => {
		await cli([
			'--glob-options={"dot":true}',
			'--config',
			fixturesPath('config-block-no-empty.json'),
			fixturesPath('glob-options'),
		]);
	...
	it('output a message when unsupported --glob-options provided', async () => {
		await cli([
			'--glob-options={"cwd":"."}',
			'--config',
			fixturesPath('config-block-no-empty.json'),
