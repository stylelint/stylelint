...
describe('buildCLI', () => {
	...
	it('flags.mergeRules', () => {
		expect(buildCLI(['--merge-rules']).flags.mergeRules).toBe(true);
	});
	...
	it('flags.rule', () => {
		expect(buildCLI(['--rule', 'baz: true', '--rule', 'qux: false']).flags.rule).toStrictEqual([
			'baz: true',
			'qux: false',
		]);
	});
	...
	it('flags.rules', () => {
		expect(buildCLI(['--rules', 'baz: false, qux: true']).flags.rules).toBe(
			'baz: false, qux: true',
		);
	});
...
describe('CLI', () => {
	...
	describe('--rule', () => {
		it('adds the rules into the config', async () => {
			await cli([
				'--print-config',
				'--rule',
				' "color-named": "never" ',
		...
		it('throws if the value is missing', async () => {
			await expect(
				cli(['--print-config', '--rule', 'color-no-invalid-hex', fixturesPath('invalid-hex.css')]),
			).rejects.toThrow('--rule argument is malformed.');
		});

		it('throws if the value is an array', async () => {
			await expect(
				cli(['--print-config', '--rule', '[]', fixturesPath('invalid-hex.css')]),
			).rejects.toThrow('--rule argument is malformed.');
		});

		it("throws if the name doesn't match the pattern", async () => {
			await expect(
				cli(['--print-config', '--rule', '"foo-": true', fixturesPath('invalid-hex.css')]),
			).rejects.toThrow('--rule argument is malformed.');
		});

		it('ignores --config if --merge-rules is absent', async () => {
			await cli([
				'--print-config',
				'--rule',
				'"color-named": ["never"]',
			...
			expect(process.stdout.write).toHaveBeenCalledTimes(1);
			expect(process.stdout.write).toHaveBeenLastCalledWith(
				JSON.stringify({ rules: { 'color-named': ['never'] } }, null, '  '),
			);
		});
	});

	describe('--rules', () => {
		it('adds rules in the right order', async () => {
			await cli([
				'--print-config',
				'--rules',
				'"color-named": ["never"], "color-no-invalid-hex": [true]',
				fixturesPath('invalid-hex.css'),
			]),
				expect(process.exitCode).toBeUndefined();
		...
		it('is ignored if --rule is passed', async () => {
			await cli([
				'--print-config',
				'--rule',
				'"color-named": ["never"]',
				'--rules',
				'"color-named": ["always-where-possible"], "color-no-invalid-hex": [true]',
				fixturesPath('invalid-hex.css'),
			]);
