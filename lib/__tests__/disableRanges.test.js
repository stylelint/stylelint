...
it("disable rule without re-enabling", () => {
  return testDisableRanges("/* stylelint-disable foo-bar */\na {}").then(result => {
    ...
    return testDisableRanges("/* stylelint-disable selector-combinator-space-before */\n" + "a {}").then(result => {
      expect(result.stylelint.disabledRanges).toEqual({
        all: [],
...
it("disable single line multiple rules", () => {
  return testDisableRanges("b {}\n\na {} /* stylelint-disable-line block-no-empty, blergh */").then(result => {
    expect(result.stylelint.disabledRanges).toEqual({
      all: [],
      "block-no-empty": [{
        start: 3,
...
it("SCSS // line-disabling comment", () => {
  ...
  return postcss().use(assignDisabledRanges).process(scssSource, { syntax: scss }).then(result => {
    expect(result.stylelint.disabledRanges).toEqual({
      all: [],
...
it("Less // line-disabling comment", () => {
  ...
  return postcss().use(assignDisabledRanges).process(lessSource, { syntax: less }).then(result => {
    expect(result.stylelint.disabledRanges).toEqual({
      all: [],
      "declaration-no-important": [{
        start: 2,
