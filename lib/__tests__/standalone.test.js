...
describe("standalone with two file-specific globs", () => {
  const twoCsses = [
    `${fixturesPath}/e*y-block.*`,
    `${fixturesPath}/invalid-h*.css`
  ];
...
it("standalone without input css and file(s) should throw error", () => {
  ...
  expect(() => standalone({ config: configBlockNoEmpty })).toThrow(
    expectedError
  );
});
...
describe("standalone passing code with syntax error", () => {
  ...
  it("(CssSyntaxError) in warning text", () => {
    expect(results[0].warnings[0].text.indexOf(" (CssSyntaxError)")).not.toBe(
      -1
    );
  });
});
...
it("unknown custom syntax option", () => {
  return standalone({
    ...
    .catch(err => {
      expect(err.message).toBe(
        "Cannot resolve custom syntax module unknown-module"
      );
    });
});
...
it("unknown formatter option", () => {
  return standalone({
    ...
    .catch(err => {
      expect(err.message.indexOf("You must use a valid formatter option")).toBe(
        0
      );
    });
});
...
describe("standalone with different configs per file", () => {
  ...
  it("correct warning for B", () => {
    ...
    expect(resultB.warnings[0].text.indexOf("Unexpected empty block")).not.toBe(
      -1
    );
  });
...
describe("standalone with config locatable from process.cwd not file", () => {
  ...
  beforeEach(() => {
    return standalone({
      files: [
        replaceBackslashes(path.join(__dirname, "./fixtures/empty-block.css"))
      ]
    }).then(data => (results = data.results));
  });
