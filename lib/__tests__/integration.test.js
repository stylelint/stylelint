...
describe("integration test expecting warnings", () => {
  ...
  it("block-opening-brace-newline-after - string primary option", () => {
    expect(result.messages[0].text).toBe("Expected newline after \"{\" (block-opening-brace-newline-after)")
    expect(result.messages[0].severity).toBe("error")
  })

  it("declaration-block-properties-order - nested array primary option", () => {
    expect(result.messages[1].text).toBe("Expected \"display\" to come before \"height\" (declaration-block-properties-order)")
    expect(result.messages[1].severity).toBe("error")
  })

  it("declaration-property-unit-blacklist - object primary option", () => {
    expect(result.messages[2].text).toBe("Unexpected unit \"px\" for property \"width\" (declaration-property-unit-blacklist)")
    expect(result.messages[2].severity).toBe("error")
  })
  ...
  it("color-no-invalid-hex - true primary option", () => {
    ...
    expect(result.messages[4].text).toBe("You made a mistake")
    expect(result.messages[4].severity).toBe("warning")
  })

  it("function-blacklist - array primary option", () => {
    expect(result.messages[5].text).toBe("Unexpected function \"calc\" (function-blacklist)")
    expect(result.messages[5].severity).toBe("error")
  })
})
