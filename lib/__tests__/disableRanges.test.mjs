...
it('disable single line all rules, in a rule node', async () => {
	const result = await testDisableRanges(stripIndent`
		/* offset by one line */
		a,/* stylelint-disable-line */
		b/* stylelint-disable-line */,
		c,/* stylelint-disable-line */
		long-selector/* stylelint-disable-line */,
		/* stylelint-disable-line */long-selector,
		d(/* stylelint-disable-line */),
		e/* stylelint-disable-line */ {}
	`);
...
it('disable single line all rules, in an at rule node', async () => {
	const result = await testDisableRanges(stripIndent`
		/* offset by one line */
		@a/* stylelint-disable-line */
		/* stylelint-disable-line*/tty,
		tv/* stylelint-disable-line*/,
		(min-width: 20px/* stylelint-disable-line*/),
		screen,/* stylelint-disable-line*/
		print/* stylelint-disable-line*/ {}
	`);
...
it('disable single line all rules, in a declaration node', async () => {
	const result = await testDisableRanges(stripIndent`
		.foo{
...
it('disable (with multi-line description)', async () => {
	const result = await testDisableRanges(
		`/* stylelint-disable
      --
      Description
...
it('disable comments without source location', async () => {
	const result = postcss()
		.use((root) => {
			// Insert nodes without a `source` value.
			root.append({ text: 'b /* stylelint-disable-line */ {}' });
			root.append({ text: 'c { color: rgb(255 0 /* stylelint-disable-line */ 0); }' });
			root.append({ text: '@media (width: 400px /* stylelint-disable-line */) d {} }' });
			root.append({ text: '/* stylelint-disable */' });
			root.append({ text: 'e {}' });
		})
		.process(
			`/* stylelint-disable */
	...
	assignDisabledRanges(result.root, result);
...
function expectDisableRange(actual, expected) {
	...
	if ('node' in actual) {
		expectedMutable.node = actual.node;
	}
