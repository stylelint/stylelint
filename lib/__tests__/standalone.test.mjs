...
test('option validation | default true', async () => {
	const {
		results: [result],
	} = await standalone({
		files: 'empty-block.css',
	...
	expect(result.invalidOptionWarnings).toHaveLength(1);
});

test('option validation | option false', async () => {
	const {
		results: [result],
	} = await standalone({
		files: 'empty-block.css',
...
test('option validation | config false', async () => {
	const {
		results: [result],
	} = await standalone({
		files: 'empty-block.css',
