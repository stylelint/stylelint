...
describe('mergeRule', () => {
	it('ignores configFile when config is passed and mergeRules is disabled (code)', async () => {
		const { results } = await standalone({
			code: 'a { color: #000; background: #ZZ }',
			config: { rules: { 'color-named': ['always-where-possible'] } },
			mergeRules: false,
			configFile: `${fixturesPath}/config-color-no-invalid-hex.json`,
		});
		const array = results[0].warnings;
		const [colorNamed] = array;

		expect(array).toHaveLength(1);
		expect(colorNamed.rule).toBe('color-named');
	});

	it('ignores configFile when config is passed and mergeRules is disabled (files)', async () => {
		const { results } = await standalone({
			config: { rules: { 'color-named': ['always-where-possible'] } },
			mergeRules: false,
			configFile: `${fixturesPath}/config-color-no-invalid-hex.json`,
			files: [`${fixturesPath}/merge-rules.css`],
		});
		const array = results[0].warnings;
		const [colorNamed] = array;
	...
	it('merges rules (code)', async () => {
		...
		const array = results[0].warnings;
		const [colorNamed, colorNoInvalidHex] = array;

		expect(array).toHaveLength(2);
		expect(colorNamed.rule).toBe('color-named');
		expect(colorNoInvalidHex.rule).toBe('color-no-invalid-hex');
	});

	it('merges rules (files)', async () => {
		const { results } = await standalone({
			config: { rules: { 'color-named': ['always-where-possible'] } },
			mergeRules: true,
			configFile: `${fixturesPath}/config-color-no-invalid-hex.json`,
