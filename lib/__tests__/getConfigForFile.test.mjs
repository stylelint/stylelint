...
test('loads TypeScript config concurrently for different files', async () => {
	const stylelint = createStylelint();
	const fooPath = path.join(dirname, 'fixtures/getConfigForFile/ts/a/foo.css');
	const barPath = path.join(dirname, 'fixtures/getConfigForFile/ts/b/bar.css');
	const expectedConfigPath = path.join(dirname, 'fixtures/getConfigForFile/ts/stylelint.config.ts');

	const results = await Promise.all(
		Array.from({ length: 20 }, (_, index) => {
			const filePath = index % 2 === 0 ? fooPath : barPath;

			return getConfigForFile({ stylelint, searchPath: filePath, filePath });
		}),
	);

	for (const result of results) {
		expect(result.filepath).toBe(expectedConfigPath);
		expect(result.config.rules).toStrictEqual({
			'block-no-empty': [true],
		});
	}
});
