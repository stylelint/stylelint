...
describe('writing fixes to files', () => {
	...
	beforeEach(() => {
		...
		return copyFile(`${fixturesPath}/fix.css`, tempFile);
	});
	...
	it('overwrites the original file', () => {
		return standalone({
			...
			return readFile(tempFile, 'utf8').then((fileContent) => {
	...
	it("doesn't write to ignored file", () => {
		return standalone({
			...
			return readFile(tempFile, 'utf8').then((newFile) => {
				return readFile(`${fixturesPath}/fix.css`, 'utf8').then((oldFile) => {
