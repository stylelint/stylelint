...
it('two rules being disabled', async () => {
	...
	expect(
		warnings.some((w) => w.text === 'Expected indentation of 0 spaces (indentation)'),
	).toBe(true);
	expect(
		warnings.some((w) => w.text === 'Expected "COLOR" to be "color" (property-case)'),
	).toBe(true);
});
...
it('one rule being disabled and another still autofixing', async () => {
	...
	expect(
		warnings.filter((w) => w.text === 'Expected indentation of 0 spaces (indentation)').length > 0,
	).toBe(true);
	expect(
		warnings.filter((w) => w.text === 'Expected "COLOR" to be "color" (property-case)').length > 0,
	).toBe(false);
});
