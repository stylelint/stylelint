...
describe('stylelint commands', () => {
	suite('stylelint-disable', () => {
		...
		it("doesn't fix with multiple scoped stylelint-disable commands", async () => {
			const code = `
				/* stylelint-disable selector-not-notation, function-name-case */
				:not(a, div) {
					color: HSLA();
				}`;
		...
		it("doesn't fix with a scoped stylelint-disable command, but does fix other rules", async () => {
			...
			expect(result.code).toBe(`
				/* stylelint-disable function-name-case */
				a { color: #fff; color: HSLA(); }
			`);
			expect(result.report).toBe('');
		});

		it('does fix code which is out of range', async () => {
			const code = `
				:not(
					a,
					div
	...
	suite('plugin', () => {
		...
		test('meta.fixer is false', async () => {
			const config = {
