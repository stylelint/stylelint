...
describe('fix util', () => {
	...
	suite.skip('core rules + safeDisables off', () => {
	...
	suite('plugin + safeDisables off', () => {
		...
		test('scoped: with a fixer', async () => {
			const result = await standalone({
				code: stripIndent`
					.foo {}
		...
		test('scoped: without a fixer', async () => {
			const result = await standalone({
				code: stripIndent`
					.foo {}
					/* stylelint-disable plugin/selector-no-foo */
	...
	suite('plugin + safeDisables on', () => {
		test('fixer: with (disabled) + without (enabled)', async () => {
			const result = await standalone({
				code: stripIndent`
					.foo {}
					.qux {}
		...
		test('fixer: with (enabled) + without (disabled)', async () => {
			const result = await standalone({
				code: stripIndent`
					.qux {}
					.foo {}
		...
		test('without fixers', async () => {
			const result = await standalone({
				...
				config: {
					plugins: [
						fixturesPath('./plugin-selector-no-foo.mjs'),
						fixturesPath('./plugin-selector-no-qux.mjs'),
					],
					rules: {
						'plugin/selector-no-foo': true,
