...
it("doesn't fix when rules always attempt to apply fixes", async () => {
	const code = `
		.foo {}`;
	...
	const result = await standalone({
		code,
		config: {
			plugins: [fixturesPath('./plugin-fixes-always.mjs')],
			rules: {
				'plugin/fixes-always': true,
			},
		},
		fix: false,
		formatter: 'string',
...
it("doesn't fix with stylelint-disable commands when rules always attempt to apply fixes", async () => {
	const code = `
		/* stylelint-disable */
		.foo {}`;
...
it('does fix with scoped stylelint-disable commands when rules always attempt to apply fixes', async () => {
	const code = stripIndent`
		/* stylelint-disable plugin/fixes-always */
		.foo {}`;
