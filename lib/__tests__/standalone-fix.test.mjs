		...
		test('range', async () => {
			return expect(
				standalone({
					...
					`,
					config: {
						plugins: [fixturesPath('./plugin-meta-fixer-buggy.mjs')],
						rules: { 'plugin/selector-no-qux': true },
						fix: true,
					},
				}),
			).rejects.toThrow(
				'The fix callback for rule "plugin/selector-no-qux" requires either index/endIndex or start/end to be passed to the `report()` function.',
			);
		});
