...
// TODO: This test fails only on Windows for some reason. Remove the test when CJS is unsupported.
const describeforCJS = platform === 'win32' ? describe.skip : describe;

describeforCJS('standalone CJS', () => {
	let standaloneCJS;
	let options;
	let consoleWarn;

	beforeEach(async () => {
		standaloneCJS = (await import('../standalone.cjs')).default;
		options = {
			code: 'a {}',
			config: { rules: {} },
		};
	...
	it('warns about CJS API', async () => {
		await standaloneCJS({ ...options, quietDeprecationWarnings: false });
