...
describe('standalone globbing', () => {
	describe('paths with special characters', () => {
		// ref https://github.com/micromatch/picomatch#globbing-features
		const fixtureDirs = [
			`[digit]/not-digits`,
			`a(b)`,
			`with spaces`,
		...
		it.each(fixtureDirs)(`static path contains "%s"`, async (fixtureDir) => {
			...
			const { results } = await standalone({
				...
				cwd: path.join(fixturesPath, fixtureDir),
			});
	...
	describe('globbyOptions', () => {
		const dotFixturesPath = replaceBackslashes(
			new URL('./fixtures/globby-options', import.meta.url),
		);

		it('setting "dot" to true matches dot directories', async () => {
			const { results } = await standalone({
				files: '**/*.css',
				config: { rules: { 'block-no-empty': true } },
				cwd: dotFixturesPath,
			...
			expect(results).toHaveLength(1);
			expect(results[0].source).toEqual(expect.stringContaining('.dot'));
			expect(results[0].errored).toBe(true);
		});

		it('leaving "dot" unset ignores dot directories', async () => {
			const { results } = await standalone({
				files: '**/*.css',
				config: { rules: { 'block-no-empty': true } },
				cwd: dotFixturesPath,
		...
		it('throws for an unsupported option', async () => {
			await expect(
				standalone({
					files: '**/*.css',
					config: { rules: { 'block-no-empty': true } },
					cwd: dotFixturesPath,
					globbyOptions: { cwd: dotFixturesPath },
				}),
			).rejects.toThrow('unknown option "cwd"');
		});
	});

	// https://github.com/stylelint/stylelint/issues/4211
	it('glob has no + character, matched path does', async () => {
		const files = '**/glob-plus.css'; // file is in dir 'glob+chars'
	...
	it('glob contains + character, matched path does not', async () => {
		const files = '+(g)lob-contains-plus/*.css';
	...
	it('negated glob patterns', async () => {
		const files = ['negated-globs/**/*.css', '!negated-globs/ignore/**/*.css'];
	...
	describe('windows path style with backslash', () => {
		...
		it('should handle a backslash-separated file path', async () => {
			const cssGlob = path.win32.join('with spaces', 'styles.css');
	...
	describe('mixed globs and paths with special chars', () => {
		it('manual escaping', async () => {
			const cssGlob = 'got\\[braces\\] and \\(spaces\\)/*.+(s|c)ss';
