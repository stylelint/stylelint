...
describe('buildCLI', () => {
	...
	it('flags.globbyOptions', () => {
		expect(buildCLI(['--globby-options=json']).flags.globbyOptions).toBe('json');
		expect(buildCLI(['--go=json']).flags.globbyOptions).toBe('json');
	});
...
describe('CLI', () => {
	...
	it('output a message when wrong --globby-options provided', async () => {
		await cli([
			`--globby-options=stylelint-test-globby-options`,
			'--config',
			fixturesPath('default-severity-warning.json'),
			fixturesPath('empty-block.css'),
		]);
		...
		expect(process.stderr.write).toHaveBeenCalledTimes(1);
		expect(stripAnsi(process.stderr.write.mock.calls[0][0])).toContain(
			'Invalid option "--globby-options". The value "stylelint-test-globby-options" is not valid JSON object.',
		);
	});
