...
const getWarnings = (results, targetFilePath) => {
	const result = results.find((file) => file.source === targetFilePath);
...
describe('standalone cache', () => {
	...
	it('return results of cached warning file', async () => {
		await removeFile(expectedCacheFilePath);
		await fs.copyFile(invalidFile, newFileDest);

		// Return warnings when no cache
		...
		expect(getWarnings(results, newFileDest)).toHaveLength(1);
		expect(results.some((file) => isChanged(file, newFileDest))).toBe(true);

		// Return cached warnings when re-linted
		const { results: resultsNew } = await standalone(
			getConfig({
				config: {
					rules: { 'block-no-empty': [true, { severity: 'warning' }] },
				},
		...
		expect(getWarnings(resultsNew, newFileDest)).toHaveLength(1);
		expect(resultsNew.some((file) => isChanged(file, newFileDest))).toBe(false);
	});
