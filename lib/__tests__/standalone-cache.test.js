...
describe('standalone cache', () => {
	...
	it('returns result also cached warning file', async () => {
		...
		expect(results.find((file) => file.source === newFileDest).warnings).toHaveLength(1);
		...
		expect(resultsNew.find((file) => file.source === newFileDest).warnings).toHaveLength(1);
	});
