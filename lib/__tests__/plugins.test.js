...
it("config with its own plugins extending another config that invokes a plugin with a relative path", () => {
  return processorRelativeAndExtendRelative.process(cssWithFooAndBar).then(result => {
    expect(result.warnings().length).toBe(2)
...
describe("plugin using exposed rules via stylelint.rules", () => {
  ...
  it("with uppercase expectation and lowercase color", () => {
    return postcss().use(stylelint(config("upper"))).process(cssWithDirectiveLower).then(result => {
      expect(result.warnings().length).toBe(1)
  ...
  it("with uppercase expectation and uppercase color", () => {
    return postcss().use(stylelint(config("upper"))).process(cssWithDirectiveUpper).then(result => {
      expect(result.warnings().length).toBe(0)
    })
  })

  it("with lowercase expectation and uppercase color", () => {
    return postcss().use(stylelint(config("lower"))).process(cssWithDirectiveUpper).then(result => {
      expect(result.warnings().length).toBe(1)
      expect(result.warnings()[0].text).toBe("Expected \"#EEE\" to be \"#eee\" (color-hex-case)")
    })
  })
  ...
  it("with lowercase expectation and lowercase color", () => {
    return postcss().use(stylelint(config("lower"))).process(cssWithDirectiveLower).then(result => {
      expect(result.warnings().length).toBe(0)
    })
  })

  it("with uppercase expectation and lowercase color without directive", () => {
    return postcss().use(stylelint(config("upper"))).process(cssWithoutDirectiveLower).then(result => {
      expect(result.warnings().length).toBe(0)
    })
  })

  it("with uppercase expectation and uppercase color without directive", () => {
    return postcss().use(stylelint(config("lower"))).process(cssWithoutDirectiveUpper).then(result => {
      expect(result.warnings().length).toBe(0)
    })
  })
})
...
describe("module providing an array of plugins", () => {
  ...
  it("first plugin works", () => {
    return postcss().use(stylelint(config)).process("@@check-color-hex-case a { color: #eee; }").then(result => {
      expect(result.warnings().length).toBe(1)
      expect(result.warnings()[0].text).toBe("Expected \"#eee\" to be \"#EEE\" (color-hex-case)")
    })
  })
