...
async function getProcessors(config) {
	...
	const processors = config.processors.map(async (processorLookup) => {
		let processorImport;
		...
		return typeof processorImport === 'function' ? await processorImport() : processorImport;
	});

	config.processors = await Promise.all(processors);
