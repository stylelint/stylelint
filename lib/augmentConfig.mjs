...
async function addPluginFunctions(config, { quietDeprecationWarnings }) {
	...
	for (const pluginLookup of normalizedPlugins) {
		...
		if (typeof pluginLookup === 'string') {
			...
			// NOTE: This '.cjs' check is limited. Some CJS plugins may have the '.js' extension.
			if (!quietDeprecationWarnings && pluginLookup.endsWith('.cjs')) {
				console.warn(
					`"${pluginLookup}" is a CJS plugin and has been deprecated since 16.0.0. See https://stylelint.io/migration-guide/to-16`,
				);
			}
		} else {
