...
const OVERRIDE_MISSING_FILES_ERROR =
...
async function augmentConfigBasic(
	...
	if (filePath) {
		// Get or create compiled override matchers for this config.
		let compiledOverrides;

		if (augmentedConfig.overrides && configFilePath) {
			const cacheKey = `${configFilePath}::${rootConfigDir}`;

			compiledOverrides = stylelint._compiledOverridesCache.get(cacheKey);

			if (!compiledOverrides) {
				compiledOverrides = compileOverrideMatchers(augmentedConfig.overrides, rootConfigDir);
				stylelint._compiledOverridesCache.set(cacheKey, compiledOverrides);
			}
		}

		augmentedConfig = applyOverrides(augmentedConfig, rootConfigDir, filePath, compiledOverrides);
	}
 ...
 *
 * @param {StylelintConfig['overrides']} overrides
 * @param {string} rootConfigDir
...
export function compileOverrideMatchers(overrides, rootConfigDir) {
	...
	return overrides.map((override) => {
		...
		if (!files) {
			throw new Error(OVERRIDE_MISSING_FILES_ERROR);
		}
		...
		return {
			configOverrides,
			matches: (/** @type {string} */ filePath) =>
				absoluteMatcher(filePath) || basenameMatcher(filePath),
		};
	});
}

/**
 * Apply pre-compiled overrides to a file path.
 *
 * @param {StylelintConfig} config
 * @param {CompiledOverride[]} compiledOverrides
 * @param {string} filePath
 * @returns {StylelintConfig}
 */
function applyCompiledOverrides(config, compiledOverrides, filePath) {
	for (const { configOverrides, matches } of compiledOverrides) {
		if (matches(filePath)) {
 ...
 * @param {string} rootConfigDir
 * @param {string} filePath
 * @param {CompiledOverride[]} [compiledOverrides] Pre-compiled override matchers for performance
 * @returns {StylelintConfig}
 */
export function applyOverrides(fullConfig, rootConfigDir, filePath, compiledOverrides) {
	...
	if (compiledOverrides) {
		return applyCompiledOverrides(config, compiledOverrides, filePath);
	}
