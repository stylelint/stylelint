...
export async function augmentConfigFull(stylelint, filePath, cosmiconfigResult) {
	...
	augmentedConfig = await addReferenceFiles(augmentedConfig);
	augmentedConfig = await addCustomSyntax(augmentedConfig);
	...
	if (config.referenceFiles) {
		config.referenceFiles = [config.referenceFiles].flat().map((entry) => {
			if (typeof entry === 'string') {
				return absolutizeGlob(entry, configDir);
			}

			return {
				...entry,
				files: [entry.files].flat().map((glob) => absolutizeGlob(glob, configDir)),
				...(typeof entry.customSyntax === 'string'
					? { customSyntax: getModulePath(configDir, entry.customSyntax, cwd) }
					: {}),
			};
		});
	}
...
async function resolveReferenceFiles(config) {
	...
	/** @type {NonNullable<StylelintConfig['_resolvedReferenceFiles']>} */
	const resolved = [];

	for (const rawEntry of [config.referenceFiles].flat()) {
		...
		const patterns = /** @type {string[]} */ (entry.files);
		...
		resolved.push({ files, customSyntax });
	}

	config._resolvedReferenceFiles = resolved;
