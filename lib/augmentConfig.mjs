...
export async function augmentConfigFull(stylelint, filePath, cosmiconfigResult) {
	...
	augmentedConfig = await getProcessors(augmentedConfig);
 ...
 * @returns {Promise<StylelintConfig>}
 */
async function getProcessors(config) {
	if (!config.processors) {
		return config;
	}
	...
	const processorPromises = config.processors.map(async (processorLookup) => {
		...
		if (typeof processorLookup === 'string') {
			processorImport = await dynamicImport(processorLookup);
			processorImport = processorImport.default || processorImport;
		} else {
			processorImport = processorLookup;
		}

		const { name, ...functions } =
			(typeof processorImport === 'function' ? await processorImport() : processorImport) ?? {};

		if (!name) {
			throw configurationError(
				`stylelint requires processors to expose a name. The processor "${processorLookup}" is not doing this, so will not work with stylelint. Please file an issue with the processor.`,
			);
		}
	...
	(await Promise.all(processorPromises)).forEach(({ name, functions }) => {
		if (name) {
			processorFunctions[name] = functions;
		}
	});
