...
function display(data) {
	...
	const lines = rows.map((row) =>
		columns.map(({ key, align, width }) => align(row[key], width)).join(' | '),
	);
	const separator = columns
		.map(({ align, width }, index) => {
			// Cells join with ` | `, so the outer columns carry one padding column and the inner ones two
			const paddingWidth = index === 0 || index === columns.length - 1 ? 1 : 2;

			return align(':', width + paddingWidth, '-');
		})
		.join('|');
