...
const { extendConfig } = require('./augmentConfig');
...
async function standalone({
	...
	if (filePaths.length) {
		filePaths = await filterByExtensions(stylelint, fileList, filePaths);
	}
 ...
 */
async function filterByExtensions(stylelint, fileList, filePaths) {
	const { config, configBasedir, cwd, extensions, globbyOptions } = stylelint._options;
	const globCWD = globbyOptions?.cwd || cwd;
	const getConfigExtensions = async (/** @type {Config} */ c) => {
		if (!c.extends) return c.extensions;

		// see stylelint/stylelint#1993
		const directory = configBasedir || cwd;
		const cfg = await extendConfig(stylelint, c, directory, directory);
	...
	const cfgList = config ? await getConfigExtensions(config) : [];
	const extList = cfgList?.length ? cfgList : extensions?.length ? extensions : DEFAULT_EXTENSIONS;

	return filePaths.filter((p) => {
		if (directories.some((directory) => p.startsWith(directory)))
			return extList.some((extension) => p.endsWith(`.${extension}`));
