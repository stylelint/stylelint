...
const invalidScopeDisables /*: Function*/ = require('./invalidScopeDisables');
...
/** @type {(path: string, code: string) => Promise<string>} */
 ...
 * @return {Promise<StylelintStandaloneReturnValue>}
 */
module.exports = function(options) {
	...
	const ignorePattern = options.ignorePattern
		? /** @type {string[]} */ ([]).concat(options.ignorePattern.toString())
		: /** @type {string[]} */ ([]);
				...
				return stylelint
					...
					.then((postcssResult) => {
						...
						if (
							...
							// @ts-ignore TODO TYPES css property is unknown
...
function handleError(stylelint, error) {
	if (error.name === 'CssSyntaxError') {
		return createStylelintResult(stylelint, undefined, undefined, error);
	} else {
		throw error;
	}
}
