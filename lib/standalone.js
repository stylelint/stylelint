...
module.exports = function (options) {
	...
	try {
		...
		if (readError.code === FILE_NOT_FOUND_ERROR_CODE) {
			if (Array.isArray(ignorePattern) && ignorePattern.length === 0) {
				...
				if (Object.prototype.hasOwnProperty.call(pkgObject, 'stylelintignore')) {
					const pkgIgnoreFiles = pkgObject.stylelintignore;

					// If the stylelintignore property has invalid value type ignore it by
					// setting it to empty string
					if (Array.isArray(pkgIgnoreFiles)) ignorer.add(pkgIgnoreFiles);
				}
			}
		} else throw readError;
	}

	ignorer.add(ignorePattern);
