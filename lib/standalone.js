...
module.exports = function (options/*: Object */)/*: Promise<stylelint$standaloneReturnValue>*/ {
  ...
  let ignorePatterns = []
  try {
    ignorePatterns = parseStylelintIgnore(fs.readFileSync(absoluteIgnoreFilePath, "utf8"))
  } catch (readError) {
  ...
  fileList = fileList.concat(
    ...
    alwaysIgnoredGlobs.map(glob => "!" + glob),
    ignorePatterns.map(glob => {
      if (/^!/.test(glob)) return path.resolve(process.cwd(), glob.replace(/^!/, ""))
