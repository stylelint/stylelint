...
async function standalone({
	...
	/* todo better way to check for error? better way to handle 'function or error?' */
	if (typeof formatterFunction === 'object') return Promise.reject(formatterFunction);
	...
	if (!files) {
		...
		if (
			...
			return Promise.resolve(prepareReturnValue([], maxWarnings, formatterFunction, cwd));
		}
	...
	let fileList = [files].flat().map((entry) => {
		...
		if (fs.existsSync(absolutePath)) {
			...
			return tinyglobby.escapePath(normalizePath(entry));
		}
	...
	let filePaths = await tinyglobby.glob(fileList, effectiveGlobbyOptions);
