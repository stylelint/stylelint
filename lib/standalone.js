...
function handleError(
  ...
  if (error.name === "CssSyntaxError") {
    return convertCssSyntaxErrorToResult(stylelint, error);
  } else {
    throw error;
  }
}
...
function convertCssSyntaxErrorToResult(
  ...
  // Let custom processors mutate result to for example apply source maps
  ...
  return stylelint.getConfigForFile(error.file || process.cwd()).then((
    ...
    if (config.resultProcessors) {
      config.resultProcessors.forEach(resultProcessor => {
        ...
        const returned = resultProcessor(stylelintResult, error.file);
        if (returned) {
