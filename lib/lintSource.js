  ...
  return getIsIgnored.then(isIgnored => {
    ...
    return getConfig.then((result) => {
      ...
      if (existingPostcssResult) {
        return lintPostcssResult(stylelint, existingPostcssResult, config, options).then(() => existingPostcssResult)
      }
      ...
      return stylelint._getPostcssResult({
        ...
        return lintPostcssResult(stylelint, postcssResult, config, options).then(() => postcssResult)
      })
    })
  })
}
...
function lintPostcssResult(
  ...
  rules.forEach(ruleName => {
    ...
    const performRule = Promise.resolve().then(() => {
      return ruleFunction(primaryOption, secondaryOptions)(postcssRoot, postcssResult, options)
    })
