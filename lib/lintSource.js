...
module.exports = async function lintSource(stylelint, options = {}) {
	...
	if (options.cache) {
		...
		if (inputFilePath && !stylelint._fileCache.hasFileChanged(inputFilePath)) {
			const cachedPostcssResult = stylelint._fileCache.getPostcssResult(inputFilePath);

			if (cachedPostcssResult) {
				return createCachedPostcssResult(cachedPostcssResult, inputFilePath);
			}
		}
	}
	...
	if (options.cache && inputFilePath) {
		stylelint._fileCache.setPostcssResult(inputFilePath, stylelintPostcssResult);
	}
 ...
 * @returns {PostcssResult}
 */
function createCachedPostcssResult(postcssResult, filePath) {
