...
module.exports = function lintSource(
  ...
  return getIsIgnored.then(isIgnored => {
    if (isIgnored) {
      const postcssResult/*: Object*/ = options.existingPostcssResult || createEmptyPostcssResult(inputFilePath)
...
function lintPostcssResult(
  ...
  rules.forEach(ruleName => {
    const ruleFunction =
      ruleDefinitions[ruleName] || _.get(config, ["pluginFunctions", ruleName]);
...
function createEmptyPostcssResult(filePath/*:: ?: string*/)/*: emptyPostcssResultT*/ {
  return {
    root: {
      source: {
        input: { file: filePath }
