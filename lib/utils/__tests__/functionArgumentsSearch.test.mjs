...
it('works without closing parenthesis', () => {
	expect.assertions(2);

	functionArgumentsSearch('calc(1px + 3px', 'calc', (expression, expressionIndex) => {
		expect(expression).toBe('1px + 3px');
		expect(expressionIndex).toBe(5);
	});
});
...
it('works with interpolation and missing closing parenthesis', () => {
	expect.assertions(2);
