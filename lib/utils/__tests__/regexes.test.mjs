import { atRuleRegexes, descriptorRegexes, mayIncludeRegexes } from '../regexes.mjs';
...
describe('mayIncludeRegexes', () => {
	it('layerFunction matches "layer" case-insensitively', () => {
		expect(mayIncludeRegexes.layerFunction.test('layer(foo)')).toBe(true);
		expect(mayIncludeRegexes.layerFunction.test('Layer(foo)')).toBe(true);
		expect(mayIncludeRegexes.layerFunction.test('LAYER(foo)')).toBe(true);
		expect(mayIncludeRegexes.layerFunction.test('layer(foo, bar)')).toBe(true);
		expect(mayIncludeRegexes.layerFunction.test('layer(foo')).toBe(true);
		expect(mayIncludeRegexes.layerFunction.test('layer')).toBe(false);
	});
});
