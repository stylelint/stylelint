...
describe('atRuleRegexes', () => {
	...
	it('matches "import" case-insensitively', () => {
		expect(atRuleRegexes.import.test('import')).toBe(true);
		expect(atRuleRegexes.import.test('Import')).toBe(true);
		expect(atRuleRegexes.import.test('IMPORT')).toBe(true);
		expect(atRuleRegexes.import.test('imp')).toBe(false);
		expect(atRuleRegexes.import.test('import-value')).toBe(false);
	});

	it('matches "syntax" case-insensitively', () => {
		expect(atRuleRegexes.syntaxName.test('syntax')).toBe(true);
		expect(atRuleRegexes.syntaxName.test('Syntax')).toBe(true);
		expect(atRuleRegexes.syntaxName.test('SYNTAX')).toBe(true);
		expect(atRuleRegexes.syntaxName.test('syn')).toBe(false);
		expect(atRuleRegexes.syntaxName.test('syntax-value')).toBe(false);
	});
