...
describe('validateOptions for primary options', () => {
	...
	it('undefined `actual` with `possible` value and no `optional` option', () => {
		validateOptions(result, 'foo', {
			possible: [true],
			actual: undefined,
		});
	...
	it('false `actual` for report* properties', () => {
		validateOptions(result, 'reportInvalidScopeDisables', {
			possible: [true, false],
			actual: false,
		});
		expect(result.warn).toHaveBeenCalledTimes(0);
	...
	it('false `actual` for rules named report*', () => {
		validateOptions(result, 'report-bar-qux', {
			possible: [true, false],
			actual: false,
		});
		expect(result.warn).toHaveBeenCalledTimes(1);
...
describe('validateOptions for secondary options objects', () => {
	...
	it('undefined `actual` with `possible` value and an `optional` option', () => {
		validateOptions(result, 'foo', {
			possible: [true],
			actual: undefined,
			optional: true,
