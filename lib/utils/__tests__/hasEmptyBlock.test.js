...
it("hasEmptyBlock", () => {
  ...
  expect(
    postcssCheck("@supports (animation-name: test) { a { color: pink; } }")
  ).toBeFalsy();
  expect(
    postcssCheck("@document url(http://www.w3.org/) { a { color: pink; } }")
  ).toBeFalsy();
  expect(
    postcssCheck("@page :pseudo-class { a { color: pink; } }")
  ).toBeFalsy();
  expect(postcssCheck("@font-face { font-family: sans; }")).toBeFalsy();
  expect(
    postcssCheck("@keyframes identifier { 0% { top: 0; left:} }")
  ).toBeFalsy();
  ...
  expect(
    postcssCheck("@namespace url(http://www.w3.org/1999/xhtml)")
  ).toBeFalsy();
  expect(
    postcssCheck("@namespace svg url(http://www.w3.org/2000/svg)")
  ).toBeFalsy();
});
