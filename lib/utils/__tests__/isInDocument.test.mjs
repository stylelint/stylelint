...
describe('isInDocument', () => {
	...
	describe('with postcss-html parser (inline)', () => {
		it('returns false for declarations in HTML style tags', () => {
			const document = postcssHtml.parse('<style>a { color: red; --custom: blue; }</style>');
			const decls = [];
			...
			expect(decls).toHaveLength(2);
			expect(isInDocument(decls[0], { inline: true })).toBe(false);
			expect(isInDocument(decls[1], { inline: true })).toBe(false);
		});

		it('returns false for deeply nested declarations in HTML', () => {
			const document = postcssHtml.parse(`
				<style>
					@media (min-width: 600px) {
						.container {
		...
		it('handles edge case where root.parent might be undefined', () => {
			...
			// Our function should still correctly detect that the declaration
			// is within a document context through alternative means
			expect(isInDocument(decl, { inline: true })).toBe(false);
		});

		it('returns false for declarations in multiple HTML style sources', () => {
			const document = postcssHtml.parse(`
				<style>body { margin: 0; }</style>
				<div style="color: red;">content</div>
				<style>.class { padding: 10px; }</style>
			...
			expect(decls).toHaveLength(3);
			expect(isInDocument(decls[0], { inline: true })).toBe(false); // margin: 0
			expect(isInDocument(decls[1], { inline: true })).toBe(true); // color: red
			expect(isInDocument(decls[2], { inline: true })).toBe(false); // padding: 10px
		});
	});
});
