...
import isInInlineDocument from '../isInInlineDocument.mjs';
import naiveCssInJs from '../../__tests__/fixtures/postcss-naive-css-in-js.cjs';

describe('isInInlineDocument (same as isInDocument)', () => {
	it('returns false for nodes not in a document', () => {
		...
		expect(isInInlineDocument(decl)).toBe(false);
	});

	it('returns false for root nodes', () => {
		const root = postcss.parse('a { color: red; }');

		expect(isInInlineDocument(root)).toBe(false);
	});
	...
	it('returns true for nodes directly in a document', () => {
		...
		expect(isInInlineDocument(root)).toBe(true);
	});
	...
	it('returns true for nested nodes in a document', () => {
		...
		expect(isInInlineDocument(decls[0])).toBe(true); // color: red
		expect(isInInlineDocument(decls[1])).toBe(true); // --custom: blue
	});
	...
	it('returns true for deeply nested nodes in a document', () => {
		...
		expect(decls).toHaveLength(2);
		expect(isInInlineDocument(decls[0])).toBe(true); // color: red
		expect(isInInlineDocument(decls[1])).toBe(true); // display: grid
	});
	...
	it('returns true for CSS-in-JS with naiveCssInJs parser', () => {
		...
		expect(isInInlineDocument(decls[0])).toBe(true);
	});
	...
	it('returns true for multiple CSS-in-JS blocks', () => {
		...
		expect(isInInlineDocument(decls[0])).toBe(true); // color: red
		expect(isInInlineDocument(decls[1])).toBe(true); // font-size: 14px
		expect(isInInlineDocument(decls[2])).toBe(true); // --custom: blue
	});
	...
	it('returns true for nodes with document property', () => {
		...
		expect(isInInlineDocument(mockNode)).toBe(true);
	});
	...
	it('returns false for nodes with non-document document property', () => {
		...
		expect(isInInlineDocument(mockNode)).toBe(false);
	});

	it('returns true for document nodes themselves', () => {
		const document = postcss.document();

		expect(isInInlineDocument(document)).toBe(true);
	});
	...
	it('handles complex nested document structures', () => {
		...
		expect(isInInlineDocument(decl)).toBe(true);
	});
	...
	describe('with postcss-html parser', () => {
		...
		it('returns true for custom properties in HTML style attributes', () => {
			...
			expect(decls).toHaveLength(2);
			expect(isInInlineDocument(decls[0])).toBe(true); // --theme-color
			expect(isInInlineDocument(decls[1])).toBe(true); // color
		});
	});
});

describe('isInInlineDocument (differences with isInDocument)', () => {
	it('returns true for declarations in HTML style tags', () => {
		...
		expect(decls).toHaveLength(2);
		expect(isInInlineDocument(decls[0])).toBe(false);
		expect(isInInlineDocument(decls[1])).toBe(false);
	});
	...
	it('returns true for declarations in multiple HTML style sources', () => {
		...
		expect(decls).toHaveLength(3);
		expect(isInInlineDocument(decls[0])).toBe(false); // margin: 0
		expect(isInInlineDocument(decls[1])).toBe(true); // color: red
		expect(isInInlineDocument(decls[2])).toBe(false); // padding: 10px
	});
});
