...
test('without node', () => {
	...
	expect(emitWarning).toHaveBeenCalledWith(
		expect.stringMatching(/Omitting `node` in `report` is deprecated \("foo"\)\./),
		expect.objectContaining({
			code: 'stylelint:007',
			detail: 'Please pass a `node` value to the `report` utility of "foo".',
...
test('with line', () => {
	...
	expect(emitWarning).toHaveBeenCalledWith(
		expect.stringMatching(/`line` in `report` is deprecated \("foo"\)\./),
		expect.objectContaining({
			code: 'stylelint:007',
			detail: 'Please use `index` and `endIndex` in the `report` utility of "foo" instead.',
			type: 'DeprecationWarning',
		}),
	);
});
...
test('with index without endIndex', () => {
	...
	expect(emitWarning).toHaveBeenCalledWith(
		expect.stringMatching(/Partial position information in `report` is deprecated \("foo"\)\./),
		expect.objectContaining({
			code: 'stylelint:007',
			detail: 'Please pass both `index` and `endIndex` to the `report` utility of "foo".',
			type: 'DeprecationWarning',
		}),
	);
});
	...
	it('has no start', () => {
		...
		expect(emitWarning).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				code: 'stylelint:007',
				detail:
					'Please pass both a valid `start` and `end` value to the `report` utility of "foo".',
				type: 'DeprecationWarning',
			}),
		);
	});
