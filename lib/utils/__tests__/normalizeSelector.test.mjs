...
it('sorts with combinator', () => {
	expect(normalizeSelector(parseSelector('.bar.foo > .foo#bar')).toString()).toBe(
		'.bar.foo>#bar.foo',
	);
	expect(normalizeSelector(parseSelector('.bar.foo + .foo#bar')).toString()).toBe(
		'.bar.foo+#bar.foo',
...
it('sorts pseudo and pseudo-element', () => {
	expect(normalizeSelector(parseSelector('.bar.foo::before:hover {}')).toString()).toBe(
		'.bar.foo:hover::before {}',
	);
	expect(normalizeSelector(parseSelector('a:hover:focus {}')).toString()).toBe('a:focus:hover {}');
});

it('sorts attribute selectors', () => {
	expect(normalizeSelector(parseSelector('a[foo]#baz.bar {}')).toString()).toBe(
		'a#baz.bar[foo] {}',
	);

	expect(normalizeSelector(parseSelector('div[foo]#bar.baz:hover::before')).toString()).toBe(
		'div#bar.baz[foo]:hover::before',
	);

	expect(normalizeSelector(parseSelector('[foo]:hover#bar.baz::before:hover')).toString()).toBe(
		'#bar.baz[foo]:hover:hover::before',
	);
});
