...
describe('compound selector', () => {
	it('sorts class names alphabetically', () => {
		expect(normalizeSelector(parseSelector('.a.b')).toString()).toBe('.a.b');
		expect(normalizeSelector(parseSelector('.b.a')).toString()).toBe('.a.b');
	});

	it('sorts pseudo-elements alphabetically', () => {
		expect(normalizeSelector(parseSelector('.a.b::before {}')).toString()).toBe('.a.b::before {}');
		expect(normalizeSelector(parseSelector('.b.a::before {}')).toString()).toBe('.a.b::before {}');
	});

	it('sorts attribute selectors alphabetically', () => {
		expect(normalizeSelector(parseSelector('a#c.b[attr="foo"] {}')).toString()).toBe(
			'a#c.b[attr="foo"] {}',
		);
		expect(normalizeSelector(parseSelector('a[attr="foo"]#c.b {}')).toString()).toBe(
			'a#c.b[attr="foo"] {}',
		);
	});
	...
	it('sorts universal selector with other selectors', () => {
		expect(normalizeSelector(parseSelector('*.foo')).toString()).toBe('*.foo');
		expect(normalizeSelector(parseSelector('*.bar.foo')).toString()).toBe('*.bar.foo');
		expect(normalizeSelector(parseSelector('*.foo.bar')).toString()).toBe('*.bar.foo');
	});
});
