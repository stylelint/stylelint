...
describe("isStandardSyntaxUrl", () => {
  ...
  it("url with port", () => {
    expect(
      isStandardSyntaxUrl("https://www.domain.com:8080/file.jpg")
    ).toBeTruthy();
  });
  ...
  it("URL with protocol(http) and `$` character in single quotes", () => {
    expect(
      isStandardSyntaxUrl(
        "'http://domain.com:8080/path$path/to$to/$file$2x.ext?$1$2=$1$2'"
      )
    ).toBeTruthy();
  });
  ...
  it("sass interpolation at the start", () => {
    expect(
      isStandardSyntaxUrl("#{$sass-variable}-color/images/bg.png")
    ).toBeFalsy();
  });
  it("sass interpolation in the middle", () => {
    expect(
      isStandardSyntaxUrl("images/#{$sass-variable}-color/bg.png")
    ).toBeFalsy();
  });
  ...
  it("less interpolation at the start with single quotes", () => {
    expect(
      isStandardSyntaxUrl("'@{less-variable}-color/images/bg.png'")
    ).toBeFalsy();
  });
  it("less interpolation in the middle with single quotes", () => {
    expect(
      isStandardSyntaxUrl("'images/@{less-variable}-color/bg.png'")
    ).toBeFalsy();
  });
