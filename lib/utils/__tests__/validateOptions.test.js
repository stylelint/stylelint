...
describe("validateOptions for primary options", () => {
  ...
  it("passing string equivalence", () => {
    ...
    expect(result.warn.mock.calls).toHaveLength(0)
  })
  ...
  it("failing string equivalence", () => {
    ...
    expect(result.warn.mock.calls).toHaveLength(1)
...
describe("validateOptions for secondary options objects", () => {
  ...
  it("invalid secondary options", () => {
    ...
    expect(result.warn.mock.calls[0][0]).toEqual("Invalid value \"neveer\" for option \"foo\" of rule \"bar\"")
    expect(result.warn.mock.calls[1][0]).toEqual("Invalid value \"false\" for option \"bar\" of rule \"bar\"")
  ...
  it("possible is actual but actual is non-object", () => {
    ...
    expect(result.warn.mock.calls[0]).toHaveLength(2)
    expect(result.warn.mock.calls[0][0]).toEqual("Invalid option value 2 for rule \"foo\": should be an object")
  })
})
...
it("validateOptions for secondary options objects with subarrays", () => {
  ...
  expect(result.warn.mock.calls[0]).toHaveLength(2)
  expect(result.warn.mock.calls[0][0]).toEqual("Invalid value \"floor\" for option \"bar\" of rule \"foo\"")
})
...
describe("validateOptions for `*-no-*` rule with no valid options", () => {
  ...
  it("with options", () => {
    ...
    expect(result.warn.mock.calls[0]).toHaveLength(2)
    expect(result.warn.mock.calls[0][0]).toEqual("Unexpected option value \"foo\" for rule \"no-dancing\"")
    result.warn.mockClear()
    ...
    expect(result.warn.mock.calls[0]).toHaveLength(2)
    expect(result.warn.mock.calls[0][0]).toEqual("Unexpected option value \"false\" for rule \"no-dancing\"")
  })
})
...
describe("validateOptions with a function for 'possible'", () => {
  ...
  it("array of mixed objects and strings passes", () => {
    ...
    expect(result.warn.mock.calls[0]).toHaveLength(2)
    expect(result.warn.mock.calls[0][0]).toEqual("Invalid option \"{\"properties\":[\"one\"]}\" for rule foo")
  })
})
