...
describe('narrowFixRange', () => {
	...
	describe('insert', () => {
		...
		test('declaration with trailing semicolon', () => {
		...
		test('atrule with trailing semicolon', () => {
			const node = postcss.parse(
				'@keyframes name { from { opacity: 45% }; to { opacity: 0.7 };};',
			).first;
			const decl = node.first.first;

			decl.value = '0.45';

			const fixData = {
				range: [decl.source.start?.offset, decl.source.end?.offset],
				text: decl.toString(),
			};

			const narrowRange = narrowFixRange(decl, fixData);
			...
			).toBe('@keyframes name { from { opacity: 0.45 }; to { opacity: 0.7 };};');
		});

		test('atrule with functions with trailing semicolon', () => {
			const node = postcss.parse('@keyframes name { color: rgba(0, 0, 0, 45%); }');
			...
			expect(
				applyFixData(
					'@keyframes name { color: rgba(0, 0, 0, 45%); }',
					narrowRange.text,
					narrowRange.range,
				),
			).toBe('@keyframes name { color: rgba(0, 0, 0, 0.45); }');
		});
	});
