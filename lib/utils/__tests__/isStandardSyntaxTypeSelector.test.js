...
describe("isStandardSyntaxTypeSelector", () => {
  ...
  it("scss variable as selector", () => {
    return rules("$variable {}", func => {
      expect(isStandardSyntaxTypeSelector(func)).toBeFalsy();
    });
  });
  it("less variable as selector", () => {
    return rules("@variable {}", func => {
      expect(isStandardSyntaxTypeSelector(func)).toBeFalsy();
    });
  });
  it("percent sign before selector", () => {
    return rules("%selector {}", func => {
      expect(isStandardSyntaxTypeSelector(func)).toBeFalsy();
    });
  });
  it("variable as selector with operator", () => {
    return rules("-$variable {}", func => {
      expect(isStandardSyntaxTypeSelector(func)).toBeFalsy();
    });
  });
