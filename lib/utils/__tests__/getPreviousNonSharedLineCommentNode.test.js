...
describe("getPreviousNonSharedLineCommentNode", () => {
  it("returns undefined if there is no node before", () => {
    const root = postcss.parse(`
      a {}
    `);
    expect(getPreviousNonSharedLineCommentNode(root.nodes[0])).toBe(undefined);
  });
  ...
  it("returns the previous node if it is not a shared-line comment", () => {
    const root = postcss.parse(`
      ...
      b {}
    `);
    expect(getPreviousNonSharedLineCommentNode(root.nodes[1])).toBe(
      root.nodes[0]
    );
  });
  ...
  it("returns the node before a prior shared-line comment at the end of its line", () => {
    const root = postcss.parse(`
      ...
      b {}
    `);
    expect(getPreviousNonSharedLineCommentNode(root.nodes[2])).toBe(
      root.nodes[0]
    );
  });
  ...
  it("returns the node before two shared-line comments", () => {
    const root = postcss.parse(`
      ...
      /* comment */ b {}
    `);
    expect(getPreviousNonSharedLineCommentNode(root.nodes[3])).toBe(
      root.nodes[0]
    );
  });
});
