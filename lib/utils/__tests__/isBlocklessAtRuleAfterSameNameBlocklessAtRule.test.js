...
describe("isBlocklessAtRuleAfterSameNameBlocklessAtRule", () => {
  it("returns false with the first node", () => {
    const root = postcss.parse(`
      @import 'x.css'
    `);
    expect(isBlocklessAtRuleAfterSameNameBlocklessAtRule(root.nodes[0])).toBe(
      false
    );
  });
  ...
  it("returns false when blockless-at-rule follows a non-at-rule", () => {
    const root = postcss.parse(`
      ...
      @import 'x.css';
    `);
    expect(isBlocklessAtRuleAfterSameNameBlocklessAtRule(root.nodes[1])).toBe(
      false
    );
  });
  ...
  it("returns true when blockless-at-rule follows a same-name-blockless-at-rule with a shared-line comment", () => {
    const root = postcss.parse(`
      ...
      @import 'x.css';
    `);
    expect(isBlocklessAtRuleAfterSameNameBlocklessAtRule(root.nodes[2])).toBe(
      true
    );
  });
});
