 ...
 */
export default function resolveConfigForFile(filePath, baseConfig, fileConfigMap) {
	...
	for (const path in fileConfigMap) {
		...
		const isGlobMatch =
			fastGlob.isDynamicPattern(path) && micromatch.isMatch(filePath, `**/${path}`, { dot: true });
