...
class NoFilesFoundError extends Error {
	...
	constructor(fileList) {
		...
		if (
			...
			const scriptContents = process.env.npm_lifecycle_script || '';

			if (packageScript) {
				this.message += `

Please use escaped double quotes for a file pattern in your package.json:

        "scripts": {
                "${packageScript}": "${scriptContents.replace(/'([^']*)'/g, '\\"$1\\"')}"
        },
`;
			} else {
				this.message += `
