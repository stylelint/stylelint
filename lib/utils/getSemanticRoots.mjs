...
/** @import {ConfigSemanticFilesEntry} from 'stylelint' */

/** @type {Map<string, Root[]>} */
const cache = new Map();

/**
 * @param {(string | ConfigSemanticFilesEntry)[] | undefined} semanticFiles
 * @param {string | undefined} configBasedir
 * @returns {Promise<Root[]>}
 */
export default async function getSemanticRoots(semanticFiles, configBasedir) {
	if (!semanticFiles) {
		return [];
	}
	...
	if (!Array.isArray(semanticFiles)) {
		throw new ConfigurationError(
			'The "semanticFiles" configuration property must be an array, e.g. { "semanticFiles": ["tokens.css"] }.',
		);
	}
	...
	for (const rawEntry of semanticFiles) {
		const entry = typeof rawEntry === 'string' ? { files: [rawEntry] } : rawEntry;

		if (!entry || typeof entry !== 'object' || !entry.files) {
			throw new ConfigurationError(
				'Every entry in the "semanticFiles" configuration property must be a string or an object with a "files" property, e.g. { "semanticFiles": ["tokens.css"] }.',
			);
		}
