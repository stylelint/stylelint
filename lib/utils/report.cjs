...
function applyFix({ fix, line, result: { stylelint }, ruleName }) {
	...
	if (!validateTypes.isFunction(apply)) return { applied: false };
	...
	if (!node?.source || !validateTypes.isNumber(node.source.start?.offset) || !validateTypes.isNumber(node.source.end?.offset)) {
...
function isFixObject(value) {
	if (!validateTypes.isObject(value)) return false;
