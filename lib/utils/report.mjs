 ...
 * @returns {{range: PostcssRange, text: string, applied: false} | {applied: boolean}}
 */
function applyFix({ fix, fixedNode, line, result: { stylelint }, ruleName }) {
	const { disabledRanges, config = {}, fixersData, fixedRanges } = stylelint;

	if (!isFn(fix)) return { applied: false };

	const shouldFix = Boolean(config.fix || (config.computeReplacementText && fixedNode));
	...
	const fixData = reduceFixRange(fixedNode, {
 ...
 * @param {{range: PostcssRange, text: string}} fixData
 * @returns {{range: PostcssRange, text: string}}
 */
function reduceFixRange(node, fixData) {
	...
	const start = node.positionInside(startOffset - node.source.start.offset);
	...
	const end = node.positionInside(endOffset - node.source.start.offset);
...
/**
 * @param {PostcssRange} a
 * @param {PostcssRange} b
 * @returns {boolean}
 */
function rangesOverlap(a, b) {
	// a: ----
	// b:      ----
	if (a.end.offset < b.start.offset) return false;

	// a:      ----
	// b: ----
	if (a.start.offset > b.end.offset) return false;
