...
export default function report(problem) {
	...
	problemRangeDeprecations(problem);
 ...
 */
function problemRangeDeprecations(problem) {
	// TODO: `config.quietDeprecationWarnings` is not available in `problem`

	if (!problem.node) {
		emitDeprecationWarning(
			`Omitting \`node\` in \`report\` is deprecated ("${problem.ruleName}").`,
			'REPORT_AMBIGUOUS_POSITION',
			`Please pass a \`node\` value to the \`report\` utility of "${problem.ruleName}".`,
		);
	}

	if (!isRange(problem) && ('start' in problem || 'end' in problem)) {
		emitDeprecationWarning(
			`Partial position information in \`report\` is deprecated ("${problem.ruleName}").`,
			'REPORT_AMBIGUOUS_POSITION',
			`Please pass both a valid \`start\` and \`end\` value to the \`report\` utility of "${problem.ruleName}".`,
		);
	}
	...
	if (!hasIndices(problem) && ('index' in problem || 'endIndex' in problem)) {
		emitDeprecationWarning(
			`Partial position information in \`report\` is deprecated ("${problem.ruleName}").`,
			'REPORT_AMBIGUOUS_POSITION',
			`Please pass both \`index\` and \`endIndex\` to the \`report\` utility of "${problem.ruleName}".`,
		);
	}

	if ('line' in problem) {
		emitDeprecationWarning(
			`\`line\` in \`report\` is deprecated ("${problem.ruleName}").`,
			'REPORT_AMBIGUOUS_POSITION',
			`Please use \`index\` and \`endIndex\` in the \`report\` utility of "${problem.ruleName}" instead.`,
		);
	}
}
