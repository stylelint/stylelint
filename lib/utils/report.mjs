...
export default function report(problem) {
	...
	if (applyFix({ ...problem, line: startLine })) return;
	...
	warningProperties.fix = computeReplacementText({ ...problem, line: startLine });
...
function applyFix({ fix, line, result: { stylelint }, ruleName }) {
	...
	if (fixingIsDisabled(line, ruleName, config, disabledRanges)) return false;
 ...
 */
function computeReplacementText({ fix, line, result: { stylelint }, ruleName }) {
	if (!fix) return;

	const { disabledRanges, config = {}, fixedRanges } = stylelint;

	if (!config.computeReplacementText) return;

	if (fixingIsDisabled(line, ruleName, config, disabledRanges)) return;
 ...
 * @returns {boolean}
 */
function fixingIsDisabled(line, ruleName, config, disabledRanges) {
	if (config.rules?.[ruleName][1]?.disableFix) return true;

	if (!config.ignoreDisables && isDisabled(ruleName, line, disabledRanges)) return true;
