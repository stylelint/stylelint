...
function problemRangeDeprecations(problem) {
	...
	if (!problem.node) {
		emitDeprecationWarning(
			...
			`Please pass a \`node\` argument to the \`report\` utility of "${problem.ruleName}".`,
		);
	}
	...
	if (!isRange(problem) && ('start' in problem || 'end' in problem)) {
		emitDeprecationWarning(
			...
			'REPORT_AMBIGUOUS_POSITION',
			`Please pass both a valid \`start\` and \`end\` argument to the \`report\` utility of "${problem.ruleName}".`,
		);
	}
