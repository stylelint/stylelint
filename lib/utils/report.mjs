import { addFixerData, applyFix } from './fix.mjs';
...
export default function report(problem) {
	...
	if (problem.end?.column && problem.start) {
		const range = { start: problem.start, end: problem.end };

		if (fix) return applyFix({ result, ruleName, range, callback: fix });

		if (unfixable === true) addFixerData(result, ruleName, { range, fixed: false });
	}
