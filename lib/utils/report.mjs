...
export default function report(problem) {
	...
	const shouldFix = result.stylelint.config?.fix || false;
	...
	const startLine = line || (reportRange.start && reportRange.start.line);
	...
	if (shouldFix) {
		const fixersData =
			result.stylelint.fixersData[ruleName] || (result.stylelint.fixersData[ruleName] = []);

		fixersData.push({ range: reportRange, fixed: hasFix });
