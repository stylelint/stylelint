...
function applyFix({ fix, fixedNode, line, result: { stylelint }, ruleName }) {
	...
	if (
		!fixedNode?.source ||
		!isNumber(fixedNode.source.start?.offset) ||
		!isNumber(fixedNode.source.end?.offset)
	) {
		return { applied: false };
	}

	/** @type [number, number] */
	const fixedNodeRange = [fixedNode.source.start.offset, fixedNode.source.end.offset];
