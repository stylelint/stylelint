...
/** @typedef {import('file-entry-cache').FileDescriptor["meta"] & { hashOfConfig?: string, postcssResult?: PostcssResult }} CacheMetadata */
...
class FileCache {
	...
	/**
	 * @param {string} absoluteFilepath
	 * @return {PostcssResult|undefined}
	 */
	getPostcssResult(absoluteFilepath) {
		const descriptor = this._fileCache.getFileDescriptor(absoluteFilepath);
		/** @type {CacheMetadata} */
		const meta = descriptor.meta || {};

		return meta.postcssResult;
	}
	 ...
	 */
	setPostcssResult(absoluteFilepath, postcssResult) {
		const descriptor = this._fileCache.getFileDescriptor(absoluteFilepath);
		/** @type {CacheMetadata} */
		const meta = descriptor.meta || {};
		...
		meta.postcssResult = postcssResult;
	}
