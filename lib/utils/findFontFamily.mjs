...
export default function findFontFamily(value) {
	...
	valueNodes.walk((valueNode, index, nodes) => {
		...
		if (isValidFontSize(valueNode.value) && !fontFamilyKeywords.has(valueLowerCase)) {
			return;
		}
		...
		// if its next node is a comma or it is already the last node, then treat it as a `font-family`
		// keyword. Otherwise treat it as a `font-size` keyword.
		if (
