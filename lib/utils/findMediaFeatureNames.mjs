...
function topLevelTokenNodes(node) {
	...
	components
		.filter((component) => component && isTokenNode(component))
		.forEach((component, index, tokenNodes) => {
			...
			if (component.value[0] === TokenType.Ident) {
				const nextComponent = tokenNodes[index + 1];

				if (
					nextComponent &&
					nextComponent.value[0] === TokenType.Delim &&
					nextComponent.value[4].value === '.'
				) {
					return;
				}
			}
