...
testRule({
	...
	accept: [
		...
		{
			code: 'a { b, & { i { color: red; } } }',
		},
		{
			code: 'a { i { color: red; &:hover { color: red; } } } b { i { color: red; &:hover { color: red; } } }',
		},
		{
			code: stripIndent`
				.tng-button {
					&:where(:not(.is-md, .is-lg)),
					.is-sm {
						.tng-icon {
							color: red;
						}
			...
			description: 'See https://github.com/stylelint/stylelint/issues/8849',
		},
		{
			code: stripIndent`
				nav {
			...
			description: 'See https://github.com/stylelint/stylelint/issues/8567',
		},
		{
			code: '.foo.bar { color: red; } .bar.foo { color: red; }',
		},
	],
	...
	reject: [
		...
		},
		{
			...
			message: messages.expected('b', '& > b', 1, '', 'a > b'),
			line: 1,
			column: 29,
			endLine: 1,
			endColumn: 30,
		...
		{
			code: '#foo .bar.baz { color: red; } .baz.bar { color: red; }',
			message: messages.expected('.baz.bar', '#foo .bar.baz', 1),
			line: 1,
			column: 31,
			endLine: 1,
			endColumn: 39,
		...
		{
			code: 'a { b:first-of-type { &:hover i::after { color: red; } } b:last-of-type { i::after { color: red; } } }',
			message: messages.expected(
				'i::after',
				'&:hover i::after',
				1,
				':is(a b:last-of-type) i::after',
				':is(a b:first-of-type):hover i::after',
			),
			line: 1,
			column: 75,
			endLine: 1,
		...
		},
		{
			...
			warnings: [
				{
					message: messages.expected('a', '.foo.bar &', 1, '', '.foo.bar a'),
					line: 1,
					column: 34,
					endLine: 1,
					endColumn: 35,
				...
				{
					message: messages.expected('.foo &', '.foo.bar &', 1, '.foo a', '.foo.bar a'),
					line: 1,
					column: 50,
					endLine: 1,
					endColumn: 56,
		...
		},
		{
			...
			message: messages.expected('& h2', '& h2', 1, 'h1 h2', '.foo h2'),
			line: 1,
			column: 47,
			endLine: 1,
			endColumn: 51,
...
testRule({
	...
	reject: [
		{
			code: 'css` &:hover { a { color: red; } } \n a { color: red; } `;',
			message: messages.expected('a', 'a', 1, '', '&:hover a'),
		},
		{
			code: 'css` &.active { a { color: red; } } \n a { color: red; } `;',
			message: messages.expected('a', 'a', 1, '', '&.active a'),
		},
	],
});
