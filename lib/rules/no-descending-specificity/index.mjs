...
const messages = ruleMessages(ruleName, {
	expected: (prior, subsequent, line, priorResolved, subsequentResolved) => {
		prior = `"${prior}"${priorResolved ? ` ("${priorResolved}")` : ''}`;
		subsequent = `"${subsequent}"${subsequentResolved ? ` ("${subsequentResolved}")` : ''}`;

		return `Expected selector ${prior} to come before selector ${subsequent}, at line ${line}`;
	},
});
...
/** @typedef {{ selector: string, selectorStr: string, specificity: import('@csstools/selector-specificity').Specificity, line: number }} Entry */
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		function checkSelector(resolvedSelectorNode, selectorNode, ruleNode, comparisonContext) {
			...
			const { selector: selectorStr, index, endIndex } = getStrippedSelectorSource(selectorNode);
			const { selector: resolvedSelectorStr } = getStrippedSelectorSource(resolvedSelectorNode);

			const entry = {
				selector: resolvedSelectorStr,
				selectorStr,
				specificity,
			...
			for (const priorEntry of priorComparableSelectors) {
				if (compare(specificity, priorEntry.specificity) < 0) {
					const hasDifferentResolvedSelector =
						(resolvedSelectorStr && resolvedSelectorStr !== selectorStr) ||
						(priorEntry.selector && priorEntry.selector !== priorEntry.selectorStr);

					const message = hasDifferentResolvedSelector
						? messages.expectedResolved
						: messages.expected;

					const messageArgs = hasDifferentResolvedSelector
						? [
								selectorStr,
								priorEntry.selectorStr,
								resolvedSelectorStr,
								priorEntry.selector,
								priorEntry.line,
							]
						: [selectorStr, priorEntry.selectorStr, priorEntry.line];
