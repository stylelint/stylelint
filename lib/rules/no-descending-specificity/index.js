...
const calculateSpecificity = require('@csstools/selector-specificity');
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		function checkSelector(selectorNode, ruleNode, comparisonContext) {
			...
			const selectorSpecificity = calculateSpecificity(selectorNode);
			...
			for (const priorEntry of priorComparableSelectors) {
				if (compareSpecificity(selectorSpecificity, priorEntry.specificity) === -1) {
					report({
						ruleName,
						result,
						node: ruleNode,
...
/**
 * @typedef {ReturnType<import('@csstools/selector-specificity').default>} Specificity
 * @param {Specificity} s1
 * @param {Specificity} s2
 * @returns {number}
 */
