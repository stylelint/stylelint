		...
		root.walkDecls((decl) => {
...

			{
				...
				tokens.forEach((token, i) => {
					...
					if (!token || token[0] !== cssTokenizer.TokenType.Dimension) return;
				...
				tokens.forEach((current, i) => {
					if (!current || current[0] !== cssTokenizer.TokenType.Delim || current[4].value !== '#') return;

					const plus1 = tokens[i + 1];

					if (!plus1 || plus1[0] !== cssTokenizer.TokenType.OpenCurly) return;

					const plus2 = tokens[i + 2];

					if (!plus2 || plus2[0] !== cssTokenizer.TokenType.Delim || plus2[4].value !== '$') return;
