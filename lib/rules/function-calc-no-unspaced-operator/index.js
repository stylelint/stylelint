	...
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			function checkForOperatorInFirstNode(nodes) {
				...
				const operatorIndex =
					(firstNode.type === 'word' || -1) && findOperatorIndex(firstNode.value);
			...
			parsedValue.walk((node) => {
				...
				for (const [operatorNodeIndex, operatorNode] of node.nodes.entries()) {
...
function findOperatorIndex(string) {
	return string.search(OPERATOR_REGEX);
}
...
function isOperator(node) {
	return node && node.type === 'word' && OPERATORS.has(node.value);
}
