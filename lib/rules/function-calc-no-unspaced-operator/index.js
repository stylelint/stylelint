...
function rule(actual, secondary, context) {
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			valueParser(decl.value).walk((node) => {
				...
				function checkSymbol(symbol) {
					...
					styleSearch(styleSearchOptions, (match) => {
						...
						// 1. Ниодного пробела перед (вставка).
						// 2. Несколько пробелов без новой строки перед (удаление).
						// 3. Пробельный символ не равный новой строке или пробелу перед (замена).
						// 4. Ниодного пробела после (вставка).
						// 5. Несколько пробелов без новой строки после (удаление).
						// 6. Пробельный символ не равный новой строке или пробелу после (замена).
						...
						// const beforeOk =
						// 	(expression[index - 1] === ' ' && !isWhitespace(expression[index - 2])) ||
						// 	isNewlineBefore(expression, index - 1);
						//
						// if (!beforeOk) {
						// 	if (context.fix) {
						// 		indexesToFix.push(symbolIndex);
						// 	} else {
						// 		complain(messages.expectedBefore(symbol), decl, expressionIndex + index);
						// 	}
						// }
						...
						// if (!afterOk) {
						// 	if (context.fix) {
						// 		indexesToFix.push(symbolIndex + 1);
						// 	} else {
						// 		complain(messages.expectedAfter(symbol), decl, expressionIndex + index);
						// 	}
						// }
			...
			if (context.fix) {
				// let valueLengthChanges = 0;

				decl.value = symbolsToFix
					// .sort((a, b) => a.index - b.index)
					.reduce((fixedValue, { insert, index }) => {
						shiftIndexes(symbolsToFix, index, insert);

						return insert ? insertSpace(fixedValue, index) : removeSpace(fixedValue, index);
						// return insertSpace(fixedValue, index + valueLengthChanges++);

						// return removeSpace(fixedValue, index + valueLengthChanges--);
					}, decl.value);
			}
		});
	};
...
function removeSpace(value, index) {
