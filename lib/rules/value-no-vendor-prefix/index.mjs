	...
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			if (secondaryOptions?.ignoreValues) {
				const isIgnored = secondaryOptions.ignoreValues.some(
					(/** @type { string | RegExp } */ element) => {
						const isStringifiedRegex =
							isString(element) && element[0] === '/' && element.at(-1) === '/';
						const useRawValue = element instanceof RegExp || isStringifiedRegex;
						/**
						...
						return Boolean(testAgainstStringOrRegExp(input, element));
					},
				);
