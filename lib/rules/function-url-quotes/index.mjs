...
import { isTokenBadURL, tokenize } from '@csstools/css-tokenizer';
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		 ...
		 * According to CSS spec, unquoted URLs cannot contain:
		 * - spaces
		 * - quotes (" or ')
		 * - parentheses
		 * - certain special characters
		 *
		 * @param {string} url - The URL content (without quotes if present)
		 * @param {boolean} hasQuotes - Whether the URL is already quoted
		 * @returns {boolean} - True if quotes are required
		 */
		function requiresQuotes(url, hasQuotes) {
			...
			const [urlToken] = tokenize({ css: `url(${urlContent})` });

			if (!urlToken) return true;

			if (isTokenBadURL(urlToken)) return true;
		...
		function checkArgs(args, index) {
			...
			// Check if quotes are required for valid CSS
			const required = requiresQuotes(leftTrimmedArgs, hasQuotes);

			// If quotes are required, don't complain about them being present
			if (required && hasQuotes) return;
