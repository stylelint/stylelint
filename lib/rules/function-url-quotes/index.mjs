...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		 ...
		 */
		function fixer(node, fn, messageType, parsedValue) {
			return () => {
				switch (messageType) {
					case 'expected':
						addQuotes(fn);
		...
		/** @typedef {[Declaration|AtRule, FunctionNode, keyof messages]} ArgsArray */
		...
		function complain(node, source, startIndex) {
			...
			const parsedValue = functionArgumentsSearch(source, /^url$/i, (args, index, funcNode) => {
				const object = checkArgs(args, startIndex + index, funcNode);

				if (object) {
					const { messageType, ...problem } = object;

					reportQueue.push([problem, [node, funcNode, messageType]]);
				}
			});
		 ...
		 * @param {number} index
		 * @param {FunctionNode} funcNode
		 * @returns {Omit<Problem, 'node'> & { messageType: keyof messages } | undefined}
		 */
