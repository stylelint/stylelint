...
const messages = ruleMessages(ruleName, {
	expected: `Expected quotes around url function argument`,
	rejected: `Unexpected quotes around url function argument`,
});
		...
		function complain(node, source, startIndex) {
			functionArgumentsSearch(source, /^url$/i, (args, index, funcNode, parsedValue) => {
				...
				if (object) {
					...
					const messageArgs = [string.value];
					const fix = getFix(node, funcNode, messageType, parsedValue);

					report({ ...rest, node, fix, message, messageArgs, result, ruleName });
				}
			});
		}
