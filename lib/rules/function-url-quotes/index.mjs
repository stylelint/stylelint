...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		function checkArgs(args, index) {
			...
			if (hasQuotes && requiresQuotes(trimmedArgs)) return;
		...
		function hasUrlModifier(funcNode) {
			const stringIndex = funcNode.nodes.findIndex((n) => n.type === 'string');

			if (stringIndex !== -1) {
				return funcNode.nodes.slice(stringIndex + 1).some((n) => n.type !== 'space');
			}

			return funcNode.nodes.some((n) => n.type === 'word' && /\s/.test(n.value));
		}
	};
};
