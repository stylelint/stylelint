...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		 ...
		 */
		function quotesRequired(url) {
			// Check for characters that require quoting
			// Spaces, quotes, parentheses, and other special characters
			return /[\s"'()]/.test(url);
		}
		...
		function checkArgs(args, index) {
			...
			let urlContent = leftTrimmedArgs;

			if (hasQuotes) {
				const quote = leftTrimmedArgs[0];

				if (quote) {
					const endQuoteIndex = leftTrimmedArgs.lastIndexOf(quote);

					if (endQuoteIndex > 0) {
						urlContent = leftTrimmedArgs.slice(1, endQuoteIndex);
					}
				}
			}

			// Check if quotes are required for valid CSS
			const required = quotesRequired(urlContent);
