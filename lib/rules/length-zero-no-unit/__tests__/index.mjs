...
testRule({
	ruleName,
	customSyntax: 'postcss-scss',
	config: [true, { ignorePreludeOfAtRules: ['function', /^mix/] }],
	accept: [
		{
			code: '@mixin foo($x: 0px) { width: $x; }',
		},
		{
			code: '@mixing bar($x: 0px) { height: $x; }',
		},
		{
			code: '@function baz($x: 0px) { @return $x; }',
		},
	],
	reject: [
		{
			code: '@min foo($x: 0px) { width: $x; }',
			fixed: '@min foo($x: 0) { width: $x; }',
			message: messages.rejected,
			line: 1,
			column: 15,
			endLine: 1,
