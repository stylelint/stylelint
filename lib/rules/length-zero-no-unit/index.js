	...
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			if (decl.prop.toLowerCase() === 'flex') {
				const clearedNodes = parsedValue.nodes.filter((node) => node.type !== 'space');
				const firstValueNodeIndex =
					clearedNodes.findIndex((node) => node.type === 'div' && node.value === ':') + 1;
				const ignoredValueNodes = clearedNodes.slice(firstValueNodeIndex, firstValueNodeIndex + 2);

				ignoredValueNodes.forEach((node) => (ignorableIndexes[node.sourceIndex] = true));
			}
