...
const MakeMutableFontFamilies = (fontFamilies, decl) => {
	...
	fontFamilies.forEach((fontFamily, idx) => {
		const newNode = {
			...
			hasQuotes: 'quote' in fontFamily,
			resetIndexes: (/** @type {number} */ offset) => {
...
const rule = (primary, _secondary, context) => {
	return (root, result) => {
		...
		root.walkDecls(/^font(-family)?$/i, (decl) => {
			let fontFamiliess = MakeMutableFontFamilies(findFontFamily(decl.value), decl);

			if (fontFamiliess.length === 0) {
				return;
			}

			for (const fontFamilyNode of fontFamiliess) {
		...
		function checkFamilyName(fontFamilyNode, decl) {
			const hasQuotes = fontFamilyNode.hasQuotes;
			const family = fontFamilyNode.name;
			const rawFamily = hasQuotes ? `"${family}"` : family;
