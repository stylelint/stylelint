...
const rule = (primary, _secondary, context) => {
	return (root, result) => {
		...
		root.walkDecls(/^font(-family)?$/i, (decl) => {
			...
			const fixFamilyNames = [];

			for (const fontFamilyNode of fontFamilies) {
				const fixFamilyName = checkFamilyName(fontFamilyNode, decl);

				if (fixFamilyName) {
					fixFamilyNames.push(fixFamilyName);
				}
			}
			...
			let typeOfQuote = "'";
			const stringQuotesRule =
				result.stylelint.config &&
				result.stylelint.config.rules &&
				result.stylelint.config.rules['string-quotes'];

			if (stringQuotesRule && stringQuotesRule[0] === 'double') {
				typeOfQuote = '"';
			}

			for (const fixFamilyName of fixFamilyNames) {
				for (const index of fixFamilyName.indexes) {
					if (fixFamilyName.type === 'remove') {
						decl.value = removeCharacterAtIndex(decl.value, index + indexOffset);
						indexOffset--;
					} else if (fixFamilyName.type === 'add') {
						decl.value = addCharacterAtIndex(decl.value, index + indexOffset, typeOfQuote);
						indexOffset++;
					}
				}
			}
		...
		function checkFamilyName(fontFamilyNode, decl) {
			...
			const family = rawFamily.replace(/^['"]|['"]$/g, ''); // NOTE: wth??
			...
			if (keywordSets.fontFamilyKeywords.has(family.toLowerCase()) || isSystemFontKeyword(family)) {
				if (hasQuotes) {
					if (context.fix) {
						...
						const closeIndex = openIndex + rawFamily.length - 1;

						return {
							type: 'remove',
							indexes: [openIndex, closeIndex],
						};
					}
 ...
 * @returns {string}
 */
function addCharacterAtIndex(string, index, char) {
