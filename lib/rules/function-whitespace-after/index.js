...
const rule = (primary, _secondaryOptions, context) => {
	return (root, result) => {
		...
		/**
		 * @param {import('postcss').Node} decl
		 * @param {string} value
		 * @param {number} valueIndex
		 * @param {((index: number) => void) | undefined} fix
		 */
		function check(decl, value, valueIndex, fix) {
			...
			parsed.walk((node) => {
				...
				checkClosingParen(value, node.sourceEndIndex, decl, valueIndex, node, fix);
			});
		}
		 ...
		 * @param {((index: number) => void) | undefined} fix
		 */
		function checkClosingParen(source, index, decl, valueIndex, node, fix) {
			...
			if (primary === 'always') {
				...
				report({
					...
					endIndex: valueIndex + index,
					result,
					ruleName,
				});
			} else if (primary === 'never') {
				const whitespaceMatch = source.slice(index).match(/^\s+/);
				...
				report({
					...
					node: decl,
					index: valueIndex + node.sourceIndex,
					endIndex: valueIndex + node.sourceEndIndex + whitespaceMatch[0].length,
					result,
					ruleName,
				});
			}
