...
const rule = function (on, options) {
  return (root, result) => {
    ...
    result.warn((
      `'${ruleName}' has been deprecated and in 8.0 will be removed. Instead use 'selector-max-type' with '0' as its primary option.`
    ), {
    ...
    function checkSelector(selector, rule) {
      parseSelector(selector, result, rule, selectorAST => {
        selectorAST.walkTags(tag => {
          ...
          if (ignoreChild && hasChildCombinatorBefore(tag)) {
            return
          }
