...
const messages = ruleMessages(ruleName, {
	rejected: (atRule, prelude) => `Invalid prelude "${prelude}" for at-rule "${atRule}"`,
});
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		root.walkAtRules((atRule) => {
			...
			try {
				error = lexer.matchAtrulePrelude(name, params).error;
			} catch {
				// Skip at-rules whose prelude syntax the lexer can't resolve, e.g.
				// `@when` and `@else` preludes referencing the `<boolean-condition>` type
				// See: https://github.com/stylelint/stylelint/issues/9511
				return;
			}
