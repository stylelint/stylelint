...
module.exports = function functionCommaSpaceChecker(opts) {
	opts.root.walkDecls((decl) => {
		...
		parsedValue.walk((valueNode) => {
			 ...
			 *   prevNode: ValueParserNode,
			 *   nextNode: ValueParserNode,
			 *   checkIndex: number,
			 *   nodeIndex: number
			 * }[]}
			 */
			...
			for (const [nodeIndex, node] of valueNode.nodes.entries()) {
				...
				commaDataList.push({
					commaNode: node,
					prevNode: valueNode.nodes[nodeIndex - 1],
					nextNode: valueNode.nodes[nodeIndex + 1],
					checkIndex,
					nodeIndex,
				});
			}

			for (const { commaNode, prevNode, nextNode, checkIndex, nodeIndex } of commaDataList) {
				opts.checker[opts.location]({
					...
					err: (message) => {
						...
						const [index, endIndex] = opts.location.startsWith('after')
							? [
									valueIndex + commaNode.sourceIndex + commaNode.before.length,
									nextNode ? valueIndex + nextNode.sourceEndIndex : declValue.length,
							  ]
							: [
									prevNode ? valueIndex + prevNode.sourceIndex : 0,
									valueIndex + commaNode.sourceEndIndex - commaNode.after.length,
							  ];
