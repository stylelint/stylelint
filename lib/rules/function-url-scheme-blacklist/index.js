  ...
  return (root, result) => {
    ...
    root.walkDecls(function (decl) {
      functionArgumentsSearch(decl.toString().toLowerCase(), "url", (args, index) => {
        ...
        const blacklistLowerCase = typeof blacklist === "string" ? blacklist.toLowerCase() : blacklist.join("|").toLowerCase().split("|")

        // Blacklist contains empty schemas and value don't contains in blacklist
        if (_.some(blacklistLowerCase, schemes => schemes.length) && !containsString(scheme, blacklistLowerCase)) {
          return
        }
