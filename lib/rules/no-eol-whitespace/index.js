...
function findErrorIndices(
	...
	// get index of first extra whitespace character before EOL
	// we need to only search the line, not the whole string
	const match = string.slice(0, lastEOLIndex).match(/[ \t]+\r?\n?$/);
	const eolWhitespaceIndex = match && typeof match.index === 'number' ? match.index : -1;

	if (eolWhitespaceIndex === -1) {
		return undefined;
	}

	const eolWhitespaceEndIndex = lastEOLIndex;

	// If the character before newline is not whitespace, ignore
	if (!whitespacesToReject.has(string[eolWhitespaceEndIndex - 1])) {
		return undefined;
	}
	...
	if (ignoreEmptyLines) {
		...
		// this newline, ignore
		const beforeNewlineIndex = string.lastIndexOf('\n', eolWhitespaceEndIndex - 1);

		if (beforeNewlineIndex >= 0 || isRootFirst) {
			const line = string.substring(beforeNewlineIndex, eolWhitespaceEndIndex - 1);
	...
	return [eolWhitespaceIndex, eolWhitespaceEndIndex];
}
...
const rule = (primary, secondaryOptions, context) => {
	return (root, result) => {
		...
		/**
		 * @param {[index: number, endIndex: number]} indices
		 */
		const reportFromIndices = ([index, endIndex]) => {
			report({
		...
		eachEolWhitespace(rootString, reportFromIndices, true);

		const errorIndices = findErrorIndices(rootString.length, rootString, {
			ignoreEmptyLines,
			isRootFirst: true,
		});

		if (errorIndices) {
			reportFromIndices(errorIndices);
		}

		/**
		 * Iterate each whitespace at the end of each line of the given string.
		 * @param {string} string - the source code string
		 * @param {(indices: [index: number, endIndex: number]) => void} callback - callback the whitespace index at the end of each line.
		...
		function eachEolWhitespace(string, callback, isRootFirst) {
			styleSearch(
				...
				(match) => {
					const indices = findErrorIndices(match.startIndex, string, {
						ignoreEmptyLines,
						isRootFirst,
					});
