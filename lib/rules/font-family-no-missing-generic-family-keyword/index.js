...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		root.walkDecls(/^font(-family)?$/i, (decl) => {
			...
			const index = declarationValueIndex(decl) + fontFamilies[fontFamilies.length - 1].sourceIndex;
			const endIndex = index + fontFamilies[fontFamilies.length - 1].value.length;
