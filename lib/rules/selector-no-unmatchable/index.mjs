					...
					const context = {
						...
						getCompounds: () => {
							compounds ??= resolvedSelectors.nodes.flatMap((memberSelector) =>
								groupByCompoundSelectors(memberSelector, isFlattenablePseudo).map((compoundNodes) =>
...
function checkUnrepresentablePseudoElements({
	...
	selector.walkPseudos((pseudoNode) => {
		...
		if (!selectorParser.isPseudoElement(pseudoNode)) return undefined;

		const ancestors = getPseudoClassAncestors(pseudoNode);

		if (ancestors.some(isNegationOrRelationalPseudoClass)) return undefined;
 ...
 */
function isFlattenablePseudo(node) {
...
function hasMultipleCompoundCombinations(resolvedSelectors, resolvedSelector) {
	...
	resolvedSelectors.walkPseudos((pseudoNode) => {
		if (isFlattenablePseudo(pseudoNode) && pseudoNode.nodes.length > 1) {
			found = true;
