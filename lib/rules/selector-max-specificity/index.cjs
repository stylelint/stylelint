		...
		const isSelectorNodeIgnored = (node) => {
			switch (node.type) {
				...
				case 'pseudo': {
					...
					node.nodes = [];
					const emptySpecificity = selectorSpecificity.selectorSpecificity(node);
		...
		root.walkRules((ruleNode) => {
			...
			flattenNestedSelectorsForRule(ruleNode, result).forEach(({ selector, resolvedSelectors }) => {
				resolvedSelectors.forEach((resolvedSelector) => {
					...
					if (
						selectorSpecificity.compare(
							selectorSpecificity.selectorSpecificity(resolvedSelector, {
