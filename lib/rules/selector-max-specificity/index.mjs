...
const messages = ruleMessages(ruleName, {
	...
	expectedResolved: (selector, max, resolvedSelector) =>
		`Expected "${selector}" ("${resolvedSelector}") to have a specificity no more than "${max}"`,
});
			...
			resolveNestedSelectorsForRule(ruleNode, result).forEach(({ selector, resolvedSelectors }) => {
				resolvedSelectors.each((resolvedSelector) => {
					...
					if (compare(specificity, maxSpecificity) > 0) {
						...
						const { selector: resolvedSelectorStr } = getStrippedSelectorSource(resolvedSelector);
						...
						report({
							...
							result,
							node: ruleNode,
							message: hasDifferentResolvedSelector ? messages.expectedResolved : messages.expected,
							messageArgs: hasDifferentResolvedSelector
								? [selectorStr, primary, resolvedSelectorStr]
								: [selectorStr, primary],
							index,
							endIndex,
						});
