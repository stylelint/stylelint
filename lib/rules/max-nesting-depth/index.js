...
const rule = (primary, secondaryOptions) => {
	...
	function nestingDepth(node, level) {
		 ...
		 * @returns {boolean}
		 */
		function containsIgnoredPseudoClassesOnly(matches, selectors) {
			return selectors.every((selector) => {
				const pseudoRule = selector.startsWith('&:') && selector[2] !== ':' && selector.substr(2);

				if (!pseudoRule) return false;

				return Boolean(matches.indexOf(pseudoRule) !== -1);
			});
		}
		...
		if (
			...
			(isRule(node) &&
				...
				containsIgnoredPseudoClassesOnly(secondaryOptions.ignorePseudoClasses, node.selectors))
		) {
