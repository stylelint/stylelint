	...
	return (root, result) => {
		...
		eachDeclarationBlock(root, (eachDecl) => {
			...
			eachDecl((decl) => {
				...
				const indexDuplicate = declsProps.indexOf(prop.toLowerCase());

				if (indexDuplicate !== -1) {
					if (ignoreDiffValues || ignorePrefixlessSameValues) {
						// fails if duplicates are not consecutive
						if (indexDuplicate !== declsProps.length - 1) {
							if (context.fix) {
								removePreviousDuplicate(decls, prop);
					...
					if (ignoreDuplicates && indexDuplicate === declsProps.length - 1) {
						return;
					}
				...
				decls.push(decl);
				declsProps.push(prop.toLowerCase());
 ...
 * @returns {void}
 * */
function removePreviousDuplicate(declarations, property) {
	const declToRemove = declarations.find((d) => d.prop.toLowerCase() === property.toLowerCase());
