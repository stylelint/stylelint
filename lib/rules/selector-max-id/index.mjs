...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		const validOptions = validateOptions(
			...
			},
			{
				...
				possible: {
					...
					includeContextFunctionalPseudoClasses: [isString, isRegExp],
				},
				optional: true,
			},
		);
		...
		function checkSelector(selectorNode, ruleNode) {
			const count = selectorNode.reduce((total, childNode) => {
				...
				if (
					...
					(childNode.type === 'pseudo' && isIncludedContextFunctionalPseudoClasses(childNode))
				) {
		 ...
		 * @returns {boolean}
		 */
		function isIncludedContextFunctionalPseudoClasses(node) {
			return optionsMatches(secondaryOptions, 'includeContextFunctionalPseudoClasses', node.value);
		}
