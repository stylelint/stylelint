...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		function checkSelector(selectorNode, ruleNode) {
			const count = selectorNode.reduce((total, childNode) => {
				...
				if (
					...
					(childNode.type === 'pseudo' && isCheckedContextFunctionalPseudoClasses(childNode))
				) {
		 ...
		 * @returns {boolean}
		 */
		function isCheckedContextFunctionalPseudoClasses(node) {
			return optionsMatches(secondaryOptions, 'checkContextFunctionalPseudoClasses', node.value);
		}
