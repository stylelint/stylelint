...
function resolveShorthandValue(
	...
	// In the `background` shorthand, `background-size` may only appear
	// immediately after `background-position`, separated by a slash. The
	// default space-join below cannot express that (and would be wrong for
	// comma-separated layers too), so it would produce an invalid declaration.
	// Skip fixing rather than emit invalid CSS; users who want the combine can
	// opt out of the size via `ignoreLonghands: ["background-size"]`.
	// spec ref: https://drafts.csswg.org/css-backgrounds/#background
	const prefix = vendor.prefix(prefixedShorthandProperty);

	if (
		vendor.unprefixed(prefixedShorthandProperty) === 'background' &&
		transformedDeclarationNodes.has(`${prefix}background-size`)
	) {
		return undefined;
	}
