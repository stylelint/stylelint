...
const messages = ruleMessages(ruleName, {
	...
	// Message for when a longhand is redundant because the shorthand was already declared with the same value
	redundantLonghand: (longhand, shorthand) =>
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		eachDeclarationBlock(root, (eachDecl) => {
			...
			// Track declared shorthand properties and their values for detecting redundant longhands
			/** @type {Map<string, string>} */
			...
			eachDecl((decl) => {
				...
				// Check if this is a shorthand property and track it
				const shorthandLonghands = longhandSubPropertiesOfShorthandProperties.get(
					/** @type {import('stylelint').ShorthandProperties} */ (unprefixedProp),
				);

				if (shorthandLonghands) {
					// This is a shorthand property, store it with its value
					declaredShorthands.set(prop, decl.value.trim());
				}
				...
				// Check if a covering shorthand was declared earlier with the same value
				for (const shorthandProperty of shorthandProperties) {
					...
					// If the shorthand was declared and values match, report redundant longhand
					if (shorthandValue !== undefined && shorthandValue === decl.value.trim()) {
						// Fix function removes the redundant longhand declaration
						const fix = () => {
							decl.remove();
						};
						...
						report({
							...
							node: decl,
							word: decl.prop,
							message: messages.redundantLonghand,
							messageArgs: [prop, prefixedShorthandProperty],
							fix: {
								apply: fix,
								node: decl.parent,
							},
