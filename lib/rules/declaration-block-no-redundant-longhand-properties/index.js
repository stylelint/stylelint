...
const { orderedLonghandSubPropertiesOfShorthandProperties } = require('../../reference/properties');
...
const rule = (primary, secondaryOptions, context) => {
	return (root, result) => {
		...
		for (const [
			shorthand,
			longhandProps,
		] of orderedLonghandSubPropertiesOfShorthandProperties.entries()) {
			...
			eachDecl((decl) => {
				...
				for (const shorthandProperty of shorthandProperties) {
					...
					if (context.fix) {
						...
						if (declNodes !== undefined && declNodes[0] !== undefined) {
							const resolveShorthandValues = () => {
								const values = prefixedShorthandData;

								/** @type {Map<string, import('postcss').Declaration>} */
								const transformedDeclarationNodes = new Map();

								declNodes.forEach((declarationNode) => {
									transformedDeclarationNodes.set(
										declarationNode.prop.toLowerCase(),
										declarationNode,
									);
								});
								...
								return values
									.map((shorthandProp) => {
										const val = transformedDeclarationNodes.get(shorthandProp);
							...
							const newShorthandDeclarationNode = declNodes[0].clone({
								prop: prefixedShorthandProperty,
								value: resolveShorthandValues(),
							});

							declNodes[0].replaceWith(newShorthandDeclarationNode);
