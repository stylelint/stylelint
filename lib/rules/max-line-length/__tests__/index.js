...
const _21whitespaces = Array.from({ length: 21 }).fill('\u0020').join('');
const _10fullwidthChars = '一二三四五六七八九十';
...
testRule({
	...
	accept: [
		{
			code: `background: lru("${_10fullwidthChars}");`,
		},
		{
			code: `background: url("/${_10fullwidthChars}");`,
		},
	],
	...
	reject: [
		{
			code: `background: lru("/${_10fullwidthChars}");`,
			message: messages.expected(40),
			line: 1,
			column: 31,
		},
		{
			code: `background: url("禅")"/${_10fullwidthChars}";`,
			message: messages.expected(40),
			line: 1,
			column: 34,
		},
