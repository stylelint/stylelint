...
const rule = (primary, secondaryOptions, context) => {
	return (root, result) => {
		...
		function tryToPopSubString(start, end) {
			...
			const excluded = rootString.substring(
				Math.max(start, startSubString),
				Math.min(end, endSubString),
			);
		...
		function checkNewline(match) {
			...
			const rawLineLength = stringWidth(rootString.substring(match.endIndex, nextNewlineIndex));
