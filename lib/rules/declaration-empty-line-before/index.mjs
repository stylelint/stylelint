...
import fixer from '../../utils/emptyLineBeforeFixer.mjs';
...
const rule = (primary, secondaryOptions, context) => {
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			const isLast = deepEqual(decl, decl.parent?.nodes.at(-1));
			const semicolon = isLast && !decl.parent?.raws.semicolon ? 0 : 1;
			const endIndex = decl.toString().length + semicolon;

			// Fix
			const fix = fixer.bind(null, {
				node: decl,
