...
const getWordEndIndex = require('../../utils/getWordEndIndex');
const getWhitespaceEndIndex = require('../../utils/getWhitespaceEndIndex');
...
const rule = (primary, _secondaryOptions, context) => {
	...
	return (root, result) => {
		...
		function checkAfterOperator(match, params, node, fix) {
			const operatorEnd = match.startIndex + match.target.length - 1;
			...
			checker.after({
				...
				err: (m) => {
					...
					const index = paramIndex + operatorEnd + 1;
					const endIndex =
						primary === 'always'
							? paramIndex + getWordEndIndex(params, operatorEnd + 1)
							: paramIndex + getWhitespaceEndIndex(params, operatorEnd + 1, true);
