...
testRule({
	...
	accept: [
		...
		{
			code: 'a { background-color: mycolor(background, primary); }',
			description: 'deprecated keyword as unknown function argument',
		},
		{
			code: 'a { color: --foo(background); }',
			description: 'deprecated keyword as custom CSS function argument',
		},
		{
			code: 'a { overflow: myFunc(overlay); }',
			description: 'deprecated keyword as unknown function argument for non-color property',
		},
		{
			code: 'a { scrollbar-color: foo(background, bar) qux; }',
			description: 'deprecated keyword inside unknown function, non-deprecated keyword outside',
		},
		{
			code: 'a { color: foo(var(--bar, background), menu); }',
			description: 'deprecated keywords nested inside unknown function',
		},
	],
	...
	reject: [
		...
		{
			code: 'a { color: CoLoR(from InactiveCaptionText srgb r g b / 0.5); }',
			fixed: 'a { color: CoLoR(from GrayText srgb r g b / 0.5); }',
			description: 'deprecated keyword inside a CSS color function should still be flagged',
			message: messages.expected('InactiveCaptionText', 'GrayText'),
			line: 1,
			column: 23,
			endLine: 1,
		...
		{
			code: 'a { scrollbar-color: foo(background, bar) menu; }',
			fixed: 'a { scrollbar-color: foo(background, bar) canvas; }',
			description: 'deprecated keyword outside unknown function is still flagged',
			message: messages.expected('menu', 'canvas'),
			line: 1,
			column: 43,
			endLine: 1,
			endColumn: 47,
