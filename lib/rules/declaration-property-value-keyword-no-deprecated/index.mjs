		...
		root.walkDecls((decl) => {
			...
			parsedValue.walk((node) => {
				...
				if (node.type === 'function' && !colorFunctions.has(node.value.toLowerCase())) {
					// count all the inner nodes and skip them
					skip = getChildrenCount(node.nodes);
				}
 ...
 * @returns {number}
 */
function getChildrenCount(nodes) {
	return nodes.reduce((previous, current) => {
		const hasNodes = current.type === 'function' && current.nodes;
		const numberOfNodes = hasNodes ? getChildrenCount(current.nodes) : 0;

		return previous + 1 + numberOfNodes;
	}, 0);
}
