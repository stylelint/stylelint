...
const HAS_NAMED = new RegExp(
	`\\b(?:${[...new Set([...fontWeightNonNumericKeywords, ...fontWeightRelativeKeywords])].join('|')})\\b`,
	'i',
);

const HAS_NUMERIC = new RegExp(`\\b(?:${[...NUMERIC_TO_NAMED.keys()].join('|')})\\b`);
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		root.walkDecls(propertyRegexes.fontWeightAndShorthandName, (decl) => {
			if (primary === 'numeric' && !HAS_NAMED.test(decl.value)) return;

			if (primary === 'named-where-possible' && !HAS_NUMERIC.test(decl.value)) return;
