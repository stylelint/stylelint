	...
	return (root, result) => {
		const validOptions = validateOptions(
			...
			{ actual: primary },
			{
				...
				possible: {
					ignoreMediaFeatureNameValues: [validateObjectWithArrayProps(isString, isRegExp)],
				},
				optional: true,
			},
		);
		...
		/** @type {SecondaryOptions['ignoreMediaFeatureNameValues']} */
		const ignoreMediaFeatureNameValues = {};

		for (const [name, values] of Object.entries(
			secondaryOptions?.ignoreMediaFeatureNameValues ?? {},
		)) {
			ignoreMediaFeatureNameValues[name] = [values].flat();
		}

		/** @type {(name: string, value: string) => boolean} */
		function isNameOfValueIgnored(name, value) {
			const ignoredValues = ignoreMediaFeatureNameValues[name];

			return Boolean(ignoredValues && matchesStringOrRegExp(value, ignoredValues));
		}
		...
		function checkSingleToken(state, token, reporter) {
			...
			const { mediaFeatureName } = state;

			if (type === TokenType.Ident) {
				const supportedKeywords = mediaFeatureNameAllowedValueKeywords.get(mediaFeatureName);
				...
				if (supportedKeywords) {
					...
					if (isNameOfValueIgnored(mediaFeatureName, keyword)) return;
				}
			...
			const supportedValueTypes = mediaFeatureNameAllowedValueTypes.get(mediaFeatureName);
