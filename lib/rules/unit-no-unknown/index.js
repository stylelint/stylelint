...
const getNumberAndUnit = require('../../utils/getNumberAndUnit');
	...
	return (root, result) => {
		...
		function check(node, value, getIndex) {
			...
			parsedValue.walk((valueNode) => {
				...
				const numberAndUnit = getNumberAndUnit(valueNode);

				if (!numberAndUnit) {
					return;
				}

				const { number, unit } = numberAndUnit;
				...
				const valueNumberLength = number.length;

				report({
					index: getIndex(node) + valueNode.sourceIndex + valueNumberLength,
					endIndex: getIndex(node) + valueNode.sourceEndIndex,
