...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		function setWalkerState(functionNode, state) {
			...
			state.ignored = name === 'url' || optionsMatches(secondaryOptions, 'ignoreFunctions', name);
		 ...
		 * @param {import('@csstools/css-parser-algorithms').TokenNode} tokenNode
		 * @param {{ allowX: boolean }} state
		 */
		const check = (node, getIndex, tokenNode, state) => {
			const [tokenType, , , endIndex, tokenValue] = tokenNode.value;
