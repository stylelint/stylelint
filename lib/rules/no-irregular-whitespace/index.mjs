...
import atRuleRawsAfterIndex from '../../utils/atRuleRawsAfterIndex.mjs';
import atRuleRawsAfterNameIndex from '../../utils/atRuleRawsAfterNameIndex.mjs';
import atRuleRawsBetweenIndex from '../../utils/atRuleRawsBetweenIndex.mjs';
import declarationRawsBetweenIndex from '../../utils/declarationRawsBetweenIndex.mjs';
...
const rule = (primary) => {
	return (root, result) => {
		...
		root.walkAtRules((atRule) => {
			...
			validate(atRule, atRule.raws.afterName, atRuleRawsAfterNameIndex);
			validate(atRule, getAtRuleParams(atRule), atRuleParamIndex);
			validate(atRule, atRule.raws.between, atRuleRawsBetweenIndex);
			validate(atRule, atRule.raws.after, atRuleRawsAfterIndex);
		});

		root.walkRules((ruleNode) => {
			validate(ruleNode, ruleNode.raws.before, zeroIndex);
			validate(ruleNode, getRuleSelector(ruleNode), zeroIndex);
			validate(ruleNode, ruleNode.raws.between, ruleRawsBetweenIndex);
			validate(ruleNode, ruleNode.raws.after, ruleRawsAfterIndex);
		});

		root.walkDecls((decl) => {
			validate(decl, decl.raws.before, zeroIndex);
			validate(decl, decl.prop, zeroIndex);
			validate(decl, decl.raws.between, declarationRawsBetweenIndex);
