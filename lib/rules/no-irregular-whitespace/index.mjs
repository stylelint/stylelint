...
const rule = (primary) => {
	return (root, result) => {
		...
		root.walkAtRules((atRule) => {
			...
			validate(atRule, cleanQuotedWhitespace(getAtRuleParams(atRule)), atRuleParamIndex);
		...
		root.walkRules((ruleNode) => {
			...
			validate(ruleNode, cleanQuotedWhitespace(getRuleSelector(ruleNode)), zeroIndex);
		...
		root.walkDecls((decl) => {
			...
			validate(decl, cleanQuotedWhitespace(getDeclarationValue(decl)), declarationValueIndex);
		});
	};
};
 ...
 * @returns {string}
 */
function cleanQuotedWhitespace(value) {
	const parsed = valueParser(value);

	parsed.walk((node) => {
		if (node.type === 'string') {
			node.value = node.value.replace(IRREGULAR_WHITESPACES_PATTERN, ' ');
		}
	});
