	...
	return (root, result) => {
		...
		function check(node, decl, insideStyleQuery) {
			if (node.type === 'function') {
				...
				let args = node.nodes;
				let passStyleQuery = insideStyleQuery;
				...
				if (name === 'var') args = node.nodes.slice(1);
					...
					// Mark that we're inside a style query
					passStyleQuery = true;
				}

				args.forEach((arg) => check(arg, decl, passStyleQuery));
			...
			// Skip reporting if we're inside a style() query
			if (insideStyleQuery) return;
