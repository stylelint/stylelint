...
import getNestedSelectorsForRule from '../../utils/getNestedSelectorsForRule.mjs';
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		function checkSelectorListInDifferentRules(ruleNode, contextSelectorSet) {
			const flattenedNestedSelectors = getNestedSelectorsForRule(ruleNode, result).flatMap(
				({ resolvedSelectors }) => resolvedSelectors.nodes,
			);

			if (!flattenedNestedSelectors.length) return;

			const hasIgnoredSelector = flattenedNestedSelectors.some((selector) => {
				const normalized = normalizeSelector(selector.clone()).toString();
		...
		function checkSelectorsInDifferentRules(ruleNode, contextSelectorSet) {
			getNestedSelectorsForRule(ruleNode, result).forEach((flattenedSelector) => {
