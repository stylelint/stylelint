		...
		root.walkRules((rule) => {
			...
			const checkPreviousDuplicationPosition = (
				...
				if (shouldDisallowDuplicateInList) {
					// iterate through Map for checking; was this selector used in a group selector?
					contextSelectorSet.forEach((selectorLine, selector) => {
						// This parses each selector O(n^2) times: For every `selectorList`, we
						// examine every `selector` from `contextSelectorSet`
						parseSelector(selector, result, rule, (selectors) => {
							// Do not walk deep into each selector; only iterate over
							// each selector in the top list
							if (
								selectors.some((s) => {
									return String(s) === selectorList;
								})
							) {
