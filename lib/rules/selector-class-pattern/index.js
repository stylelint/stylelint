...
function hasInterpolatingAmpersand(selector) {
	for (const [i, char] of Array.from(selector).entries()) {
		...
		const prevSelector = selector.charAt(i - 1);

		if (prevSelector && !isCombinator(prevSelector)) {
			return true;
		}

		const nextSelector = selector.charAt(i + 1);

		if (nextSelector && !isCombinator(nextSelector)) {
			return true;
		}
	}
