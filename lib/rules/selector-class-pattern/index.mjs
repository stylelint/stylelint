...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		const shouldResolveNestedSelectors = Boolean(
			...
			result.stylelint.config?.languageOptions?.nestingMode === 'legacy',
		);

		// TODO: emit a deprecation warning when `secondaryOptions.resolveNestedSelectors` is used and advice using `languageOptions.nestingMode` instead
