...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		const deprecatedOptionName = 'resolveNestedSelectors';

		if (secondaryOptions && deprecatedOptionName in secondaryOptions) {
			emitDeprecationWarning(
				`The "${deprecatedOptionName}" option of the "${ruleName}" rule has been deprecated.`,
				'RESOLVE_NESTED_SELECTORS_OPTION',
				'Please avoid using the option in new configurations.',
			);
		}
