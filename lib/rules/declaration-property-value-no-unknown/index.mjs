...
import { isNullish, isRegExp, isString } from '../../utils/validateTypes.mjs';
...
import validateObjectWithNestedProps from '../../utils/validateObjectWithNestedProps.mjs';
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		const validOptions = validateOptions(
			...
			{ actual: primary },
			{
				...
				possible: {
					...
					atRulesSyntax: [validateObjectWithNestedProps(isString, isNullish)],
		...
		root.walkAtRules((atRule) => {
			...
			const { error } = forkedLexer.matchAtrulePrelude(name, params);

			if (!error) return;

			if (!('mismatchLength' in error)) return;

			const { name: errorName, rawMessage, loc } = error;

			if (errorName !== 'SyntaxMatchError') return;
			...
			report({
				message: messages.rejected(name, params),
				node: atRule,
				index: atRuleIndex + loc.start.offset,
				endIndex: atRuleIndex + loc.end.offset,
				result,
				ruleName,
			});
		});
