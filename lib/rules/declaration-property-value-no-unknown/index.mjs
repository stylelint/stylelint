 ...
 * @todo create csstree issue for shape and calc-size
 * @param {import('css-tree').CssNode} cssTreeNode
 * @returns {boolean}
 */
function containsUnsupportedFunction(cssTreeNode) {
	const functions = ['clamp', 'min', 'max', 'env', 'shape', 'calc-size'];

	return Boolean(
		find(cssTreeNode, (node) => node.type === 'Function' && functions.includes(node.name)),
	);
}
