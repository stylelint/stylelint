...
/** @typedef {{ message: import('stylelint').RuleMessage, messageArgs: string[], startOffset: number, endOffset: number }} ValidationResult */

/** @type {Map<string, ValidationResult | null>} */
const cache = new Map();
...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			const cacheKey = !isCustomProperty(prop) ? `${prop}\0${value}` : null;
			let validationResult;

			if (cacheKey) {
				validationResult = cache.get(cacheKey);
			}

			if (validationResult === undefined) {
				validationResult = getValidationResult(value, prop, lexer, typedCustomPropertyNames);

				if (cacheKey) {
					// Bounded cache with FIFO eviction when full
			...
			report({
				...
				messageArgs: validationResult.messageArgs,
				node: decl,
				index: valueIndex + validationResult.startOffset,
				endIndex: valueIndex + validationResult.endOffset,
				result,
				ruleName,
			});
		});
 ...
 * @param {string} value
 * @param {string} prop
 * @param {ReturnType<import('css-tree')['fork']>['lexer']} lexer
 * @param {Map<string, string>} typedCustomPropertyNames
 * @returns {ValidationResult | null}
 */
function getValidationResult(value, prop, lexer, typedCustomPropertyNames) {
	...
	if (mathFuncResult === 'valid' || mathFuncResult === 'skip-validation') return null;
	...
	if (mathFuncResult === 'invalid') {
		...
		if (mathFuncResultStartOffset !== -1 && mathFuncResultEndOffset !== -1) {
			startOffset = mathFuncResultStartOffset;
			endOffset = mathFuncResultEndOffset;
		}

		return {
			message: messages.rejectedMath,
			messageArgs: [prop, value.slice(startOffset, endOffset)],
			startOffset,
			endOffset,
		};
	}
	...
	try {
		...
		if (containsFunctionsNotSupportedInCSSTree(cssTreeValueNode)) return null;
	} catch {
		// Ignore parse errors for `attr()`, `if()` and custom functions
		// See: https://github.com/stylelint/stylelint/issues/8779
		if (/(?:^|[^\w-])(?:attr|if|--[\w-]+)\(/i.test(value)) return null;
	...
	if (!('mismatchLength' in error)) return null;

	const { name, rawMessage, loc } = error;

	if (name !== 'SyntaxMatchError') return null;

	if (rawMessage !== 'Mismatch') return null;
	...
	if (functionNode?.loc) {
		return {
			...
			messageArgs: [prop, value.slice(functionNode.loc.start.offset, functionNode.loc.end.offset)],
			startOffset: functionNode.loc.start.offset,
			endOffset: functionNode.loc.end.offset,
		};
	}

	return {
		message: messages.rejected,
		messageArgs: [prop, value.slice(loc.start.offset, loc.end.offset)],
