...
/** Math function names that we validate */
...
const messages = ruleMessages(ruleName, {
	...
	rejectedMath: (property, expression) =>
		`Invalid expression "${expression}" for property "${property}"`,
});
	...
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			try {
				...
				if (containsOnlyEnvFunction(cssTreeValueNode)) return;
			} catch {
 ...
 * @returns {boolean}
 */
function containsOnlyEnvFunction(cssTreeNode) {
	return Boolean(find(cssTreeNode, (node) => node.type === 'Function' && node.name === 'env'));
}
 ...
 * @param {import('postcss').Declaration} _decl - The declaration node (unused)
 ...
 */
function validateMathFunctions(value, _decl, prop, forkedLexer, typedCustomPropertyNames) {
	// If the value doesn't contain any math functions, continue with normal validation
	if (!containsMathFunction(value)) {
		return 'continue';
	}

	// Skip validation if the value contains var() or env() as we can't determine the type
	if (/\b(?:var|env)\s*\(/i.test(value)) {
		return 'skip';
	}
...
function hasInvalidTypeMix(value) {
	...
	const checkFunctionForTypeMix = (funcNode) => {
		...
		if (!MATH_FUNCTIONS.has(funcName)) {
			return false;
		}
		...
		/** @type {Array<{type: 'number' | 'dimension' | 'percentage' | 'other', node: import('@csstools/css-parser-algorithms').ComponentValue}>} */
		const operands = [];
		/** @type {Array<string>} */
		const operators = [];
		...
		for (const child of children) {
			if (isTokenNode(child)) {
				...
				if (isTokenNumber(token)) {
					operands.push({ type: 'number', node: child });
				} else if (isTokenDimension(token)) {
					operands.push({ type: 'dimension', node: child });
				} else if (isTokenPercentage(token)) {
					operands.push({ type: 'percentage', node: child });
				...
				operands.push({ type: 'other', node: child });
			}
		}
		...
		if (operators.length > 0) {
			const hasNumber = operands.some((o) => o.type === 'number');
			const hasDimensionOrPercentage = operands.some(
				(o) => o.type === 'dimension' || o.type === 'percentage',
			);
	...
	const checkNode = (node) => {
		if (isFunctionNode(node)) {
			return checkFunctionForTypeMix(node);
		}
