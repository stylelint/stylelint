...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		root.walkDecls((decl) => {
			...
			if (containsCalcSizeWithSizeKeyword(value)) return;
...
/**
 * Detect calc-size() with the `size` keyword inside its arguments.
 * If present, skip validation to avoid false positives until csstree supports it.
 *
 * @param {string} value
 * @returns {boolean}
 */
function containsCalcSizeWithSizeKeyword(value) {
	const lowerValue = value.toLowerCase();

	if (!lowerValue.includes('calc-size')) return false;

	let searchIndex = 0;

	while (searchIndex < lowerValue.length) {
		const calcSizeIndex = lowerValue.indexOf('calc-size', searchIndex);

		if (calcSizeIndex === -1) return false;

		let index = calcSizeIndex + 'calc-size'.length;

		while (index < lowerValue.length && /\s/.test(lowerValue[index])) index++;
		...
		for (let i = index; i < lowerValue.length; i++) {
			const char = lowerValue[i];

			if (char === '(') depth++;
			if (char === ')') depth--;

			if (depth === 0) {
				const args = lowerValue.slice(argsStart, i);

				if (/\bsize\b/.test(args)) return true;
		...
		// Unbalanced parentheses: fall back to a conservative check.
		if (depth > 0) return /\bsize\b/.test(lowerValue.slice(index + 1));
	}
