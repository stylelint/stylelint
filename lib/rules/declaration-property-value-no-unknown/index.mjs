...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		const validOptions = validateOptions(
			...
			{ actual: primary },
			{
				...
				possible: {
					...
					atRulesSyntax: [validateAtRulesSyntax],
					cssWideKeywords: [isString],
				},
				optional: true,
			},
		);
		...
		const atRulesSyntax = secondaryOptions?.atRulesSyntax ?? {};
		const cssWideKeywords = ['-moz-initial', ...(secondaryOptions?.cssWideKeywords ?? [])];
...
/**
 * Validate the 'atRulesSyntax' option.
 *
 * @param {unknown} value
 * @returns {boolean}
 */
function validateAtRulesSyntax(value) {
	if (!isPlainObject(value)) {
		return false;
	}

	const atRuleNames = Object.keys(value);

	atRuleNames.forEach((atRuleName) => {
		const atRuleDefinition = value[atRuleName];

		if (!isPlainObject(atRuleDefinition)) {
			return false;
		}

		if (
			'prelude' in atRuleDefinition &&
			atRuleDefinition.prelude !== null &&
			!isString(atRuleDefinition.prelude)
		) {
			return false;
		}

		if ('descriptors' in atRuleDefinition) {
			const descriptors = atRuleDefinition.descriptors;
			...
			descriptorNames.forEach((descriptorName) => {
				const descriptorValue = descriptors[descriptorName];

				if (!isString(descriptorValue)) {
					return false;
				}
			});
		}
