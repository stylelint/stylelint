		...
		const validOptions = validateOptions(
			...
			{ actual: primary },
			{
				...
				possible: {
					...
					cssWideKeywords: [validateTypes.isString],
				},
				optional: true,
			},
		);
...
function validateAtRulesSyntax(value) {
	...
	atRuleNames.forEach((atRuleName) => {
		...
		if (!validateTypes.isPlainObject(atRuleDefinition)) {
			return false;
		}
		...
		if (
			...
			!validateTypes.isString(atRuleDefinition.prelude)
		) {
			return false;
		}
		...
		if ('descriptors' in atRuleDefinition) {
			...
			if (!validateTypes.isPlainObject(descriptors)) {
				return false;
			}
			...
			descriptorNames.forEach((descriptorName) => {
				...
				if (!validateTypes.isString(descriptorValue)) {
					return false;
				}
			});
		}
