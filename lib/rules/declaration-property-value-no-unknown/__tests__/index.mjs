...
testRule({
	...
	reject: [
		...
		},
		{
			...
			message: messages.rejectedParseError('filter', 'alpha(opacity=calc(20 + 10))'),
			line: 1,
			column: 13,
			endLine: 1,
			endColumn: 41,
...
testRule({
	...
	reject: [
		{
			code: 'a { top: 10--bar; }',
			message: messages.rejected('top', '10--bar'),
			line: 1,
			column: 10,
			endLine: 1,
			endColumn: 17,
...
describe('deprecation warnings', () => {
	...
	it('emits a deprecation warning when "propertiesSyntax" option is used', async () => {
		await stylelint.lint({
			code: 'a { size: 10px; }',
			config: {
				rules: {
					[ruleName]: [true, { propertiesSyntax: { size: '<length-percentage>' } }],
				},
			},
		});

		expect(emitWarning).toHaveBeenCalledWith(
			expect.stringMatching(/deprecated.*propertiesSyntax/i),
			expect.objectContaining({ type: 'DeprecationWarning', code: 'stylelint:008' }),
		);
	});

	it('emits a deprecation warning when "typesSyntax" option is used', async () => {
		await stylelint.lint({
			code: 'a { top: --foo(5px); }',
			config: {
				rules: {
		...
		expect(emitWarning).toHaveBeenCalledWith(
			expect.stringMatching(/deprecated.*typesSyntax/i),
			expect.objectContaining({ type: 'DeprecationWarning', code: 'stylelint:008' }),
		);
	});
});
