...
const messages = ruleMessages(ruleName, {
	...
	rejected: (property) => `Deprecated property "${property}"`,
});
		...
		root.walkDecls((decl) => {
			...
			if (mappedProp) {
				...
				fix = () => {
					...
					const valueRemap = VALUE_REMAP[normalizedProp];
					const remappedValue = valueRemap?.[decl.value.toLowerCase()];

					if (remappedValue) {
						decl.value = remappedValue;
					}
				};
			} else {
