	...
	return (root, result) => {
		...
		root.walkAtRules(/^(-(moz|webkit)-)?keyframes$/i, (atRuleKeyframes) => {
			atRuleKeyframes.walkRules((keyframeRule) => {
				if (keyframeRule.selectors.length > 1) {
					keyframeRule.selector = selectorParser((selectors) => {
					...
					}).processSync(keyframeRule.selector);
				} else {
					checkSelector(
						keyframeRule.selector,
						(fixedSelector) => (keyframeRule.selector = fixedSelector),
					);
				}
