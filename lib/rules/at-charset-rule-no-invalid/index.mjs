...
const rule = (primary) => {
	return (root, result) => {
		...
		root.walkAtRules(atRuleRegexes.charsetName, (atRule) => {
			...
			// Without a semicolon, a block or whatever follows the label is swallowed into the prelude
			const missingSemicolon =
				(label && label.length !== params.length) ||
				atRule.nodes ||
				(root.last === atRule && root.raws.semicolon === false);

			if (atRule.raws.between || missingSemicolon) {
