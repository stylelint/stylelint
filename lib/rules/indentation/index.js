				...
				(match, matchCount) => {
					...
					if (afterNewlineSpace !== expectedIndentation) {
						if (context.fix) {
							...
							const legibleExpectedIndentLevel = legibleExpectation(expectedIndentLevel);
							report({
								message: messages.expected(legibleExpectedIndentLevel),
								messageArgs: [legibleExpectedIndentLevel],
								node,
