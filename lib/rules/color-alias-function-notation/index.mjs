...
const rule = (primary) => {
	return (root, result) => {
		...
		const targetFunctionCall = primary == 'with-a' ? WITHOUT_A_FUNCTION_CALL : WITH_A_FUNCTION_CALL;
		const targetFunctionName = primary == 'with-a' ? WITHOUT_A_FUNCTION_NAME : WITH_A_FUNCTION_NAME;
