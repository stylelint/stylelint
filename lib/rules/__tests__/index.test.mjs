...
describe('all rules', () => {
	test.each(ruleNames)('"%s" should have metadata', async (name) => {
		...
		const condition = isPlainObject(fixable);
		const first = condition ? Object.values(fixable).some((value) => value) : [true, undefined];
		const second = condition ? true : fixable;
		const method = condition ? 'toBe' : 'toContain';

		expect(first)[method](second);
	});
