...
describe('recommended and standard configs', () => {
	...
	cp.execSync(
		'npm install --silent --no-package-lock --no-audit --omit=peer stylelint-config-recommended stylelint-config-standard',
		{ cwd },
	);

	const configRules = (name) => Object.keys(require(path.join(cwd, 'node_modules', name)).rules);
	const recommendedRules = configRules('stylelint-config-recommended');
	const standardRules = configRules('stylelint-config-standard');

	test('recommended config is not empty', () => {
		expect(recommendedRules).not.toHaveLength(0);
	});
	...
	test.each(recommendedRules)(
		'"%s" should be included in the recommended config in the rule list doc',
		(name) => {
	...
	test.each(standardRules)(
		'"%s" should be included in the standard config in the rule list doc',
		(name) => {
			expect(rulesListDoc).toMatch(
				new RegExp(`^.+\\b${name}\\b.+\\|.+\\|\\s+✅\\s+\\|.+\\|$`, 'm'),
			);
