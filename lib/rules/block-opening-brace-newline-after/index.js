        ...
        err: m => {
          if (context.fix) {
            if (expectation.indexOf("always") === 0) {
              const index = statement.first.raws.before.search(/\r?\n/);
              if (index >= 0) {
                statement.first.raws.before = statement.first.raws.before.slice(
                  index
                );
              } else {
                statement.first.raws.before =
                  context.newline + statement.first.raws.before;
              }
              backupCommentNextBefores.delete(statement.first);
              return;
