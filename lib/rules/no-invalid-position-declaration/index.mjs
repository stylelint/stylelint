...
const rule = (primary, secondaryOptions) => {
	return (root, result) => {
		...
		/** @type {(node: import('postcss').Node) => boolean} */
		const isRuleOrDeclarationContainingAtRule = (node) =>
			isRule(node) || (isAtRule(node) && declarationContainingAtRules.has(node.name.toLowerCase()));
		...
		root.walkDecls((decl) => {
			...
			// Declarations are allowed in rules and declaration-containing at-rules (@mixin, @scope)
			if (isRuleOrDeclarationContainingAtRule(parent)) return;
			...
			if (isAtRule(parent)) {
				...
				if (nestingSupportedAtKeywords.has(atRuleName)) {
					const parentRule = findNodeUpToRoot(
						decl,
						(node) => isRuleOrDeclarationContainingAtRule(node) || isIgnoredAtRule(node),
					);
