...
const rule = (primary) => {
	return (root, result) => {
		...
		root.walkRules(/&/, (ruleNode) => {
			...
			try {
				const ast = parse(getRuleSelector(ruleNode), { context: 'selectorList', positions: true });
		...
		root.walkAtRules(atRuleRegexes.scopeName, (atRule) => {
			...
			try {
				const ast = parse(getAtRuleParams(atRule), {
					atrule: 'scope',
		...
		root.walkAtRules(atRuleRegexes.scopeName, (atRule) => {
			...
			try {
				const ast = parse(params, { atrule: 'scope', context: 'atrulePrelude', positions: true });

				walk(ast, {
					visit: 'NestingSelector',
					enter(node) {
						const index = atRuleParamIndex(atRule) + node.loc.start.offset;
						const endIndex = index + 1;
