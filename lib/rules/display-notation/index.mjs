...
const messages = ruleMessages(ruleName, {
	expected: (primary) => `Expected ${primary} display notation`,
});
	...
	return (root, result) => {
		...
		root.walkDecls(DISPLAY_PROPERTY, (decl) => {
			...
			const meaningfulTokens = tokens.filter(
				(token) => !isTokenWhiteSpaceOrComment(token) && !isTokenEOF(token),
			);
			const keywords = meaningfulTokens.filter(isTokenIdent);

			// ignore values that contain functions or other non-keyword values
			if (!keywords.length || keywords.length !== meaningfulTokens.length) return;
...
function keywordOrder(a) {
	if (DISPLAY_OUTSIDE.test(a)) {
		return 1;
	}

	if (DISPLAY_INSIDE.test(a)) {
		return 2;
	}

	if (DISPLAY_LIST_ITEM.test(a)) {
		return 3;
	}
...
/**
 * @param {import('@csstools/css-tokenizer').TokenIdent[]} tokens
 * @returns {string}
 */
...
/**
 * @param {import('@csstools/css-tokenizer').CSSToken[]} originalValue
 * @param {import('@csstools/css-tokenizer').TokenIdent} firstKeyword
 * @param {import('@csstools/css-tokenizer').TokenIdent} lastKeyword
...
function fixValue(originalValue, firstKeyword, lastKeyword, replacement) {
	...
	return stringify(
		...before,
		...replacement.map((keyword) => {
			/** @type {import('@csstools/css-tokenizer').CSSToken} */
			const token = [TokenType.Ident, keyword, -1, -1, { value: keyword }];
