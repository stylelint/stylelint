...
function insertNewline(selector, expectation) {
  let separator = null;
  if (expectation === "never-multi-line") {
    separator = ", ";
  } else {
    const reNewline = /\r/;
    separator = reNewline.test(selector) ? ",\r\n" : ",\n";
  }
  return selector
    .split(",")
    .map(o => o.trim())
        ...
        match => {
          ...
          checker.afterOneOnly({
            ...
            err: m => {
              if (context.fix) {
                rule.raws.selector
                  ? (rule.raws.selector.raw = insertNewline(
                      selector,
                      expectation
                    ))
                  : (rule.selector = insertNewline(selector, expectation));
                return;
              }
              report({
                message: m,
