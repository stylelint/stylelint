...
/** @typedef {import('stylelint').Fixer} Fixer */
				...
				if (primary === 'complex') {
					...
					if (context.fix) {
						return addFixer(result, ruleName, {
							source,
							callback: fixer,
							args: { cb: fixComplex, nodes: { root: selectorRoot, rule: ruleNode, pseudo: prev } },
						});
					}
				} else {
					...
					if (context.fix) {
						return addFixer(result, ruleName, {
							source,
							callback: fixer,
							args: { cb: fixSimple, nodes: { root: selectorRoot, rule: ruleNode, pseudo } },
							unfixable: !isFixable,
						});
					}
				}
...
/**
 * @param {Object} nodes
 * @param {Pseudo} nodes.pseudo
 * @param {RuleNode} nodes.rule
 * @param {Root} nodes.root
 */
function fixSimple({ pseudo: not, rule: ruleNode, root }) {
	...
	ruleNode.selector = root.toString();

	return getNewSource(not, ruleNode, offset);
}
 ...
 * @param {Root} nodes.root
 */
function fixComplex({ pseudo: previousNot, rule: ruleNode, root }) {
	...
	return getNewSource(previousNot, ruleNode);
}
 ...
 * @param {number} [offset]
 */
function getNewSource(pseudo, ruleNode, offset = 0) {
	assert(ruleNode.source?.start && pseudo.source?.start);
	const line = ruleNode.source.start.line + pseudo.source.start.line - 1;
	const column = pseudo.source.start.column;

	return {
		start: { line, column },
		end: { line, column: column + pseudo.toString().length + offset },
	};
}
