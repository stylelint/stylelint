		...
		root.walkRules((ruleNode) => {
			...
			selectorRoot.walkPseudos((pseudo) => {
				...
				if (primary === 'complex') {
					...
					fix = fixComplex.bind({ ruleNode, selectorRoot }, prev);
				} else {
					...
					if (isFixable) fix = fixSimple.bind({ ruleNode, selectorRoot }, pseudo);
				}
...
/**
 * @param {Pseudo} not
 * @this {{ ruleNode: Rule, selectorRoot: Root }}
 */
function fixSimple(not) {
	...
	this.ruleNode.selector = this.selectorRoot.toString();

	return getNewRange(not, this.ruleNode, offset);
}
...
function fixComplex(previousNot) {
	...
	return getNewRange(previousNot, this.ruleNode);
}
