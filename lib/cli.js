  ...
  return Promise.resolve()
    ...
    .then(options => {
      ...
      return standalone(options)
        ...
        .catch((err /*: Error & { code?: number, module?: string }*/) => {
          if (err.module) {
            if (!retry && optionsBase.fix) {
              return npmi(err.module).then(
                () => cliEngine(argv, true),
                holdError
              );
            } else {
              err.stack = `PostCSS syntax module missing: "${
                err.module
              }"\nPlease use "--fix" flag for automatic install or install it by yourself.`;
            }
          }
          holdError(err);
        });
...
function npmi(moduleId) {
  console.log("Automatic installation missing module: " + moduleId); // eslint-disable-line no-console
  return new Promise((resolve, reject) => {
    const cp = spawn("npm", ["install", moduleId], {
