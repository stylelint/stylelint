 ...
 * @property {string} [configBasedir]
 * @property {{ quiet?: any, force?: any }} configOverrides
...
const meowOptions = {
    ...
    Options:
	  ...
	  --force
	  	...
	  	Exit with exit code 0 even if errors exist. Ignores "max-warnings".
...
module.exports = (argv) => {
	...
	if (cli.flags.force) {
		optionsBase.configOverrides.force = cli.flags.force;
	}
		...
		.then((options) => {
			...
			return standalone(options)
				.then((linted) => {
					...
					if (linted.errored && !options.configOverrides.force) {
						...
						process.exitCode = options.configOverrides.force ? 0 : EXIT_CODE_ERROR;
					}
				})
				.catch(handleError);
		});
