  ...
  help: `
    ...
    Options:
      ...
      --max-warnings, --mw
        ...
        Number of warnings above which the proccess will exit with code 2.
        Useful when setting "defaultSeverity" to "warning" and want to fail the process.
...
Promise.resolve()
  ...
  .then(linted => {
    ...
    if (linted.errored) {
      ...
      const warningsCount = linted.results.reduce((count, file) => {
        return count + file.warnings.length;
      }, 0);

      if (warningsCount > maxWarnings) {
        process.stdout.write(
          `Stylelint found too many warnings:${EOL}` +
          chalk.dim(`Found ${warningsCount} (maximum: ${maxWarnings})${EOL}${EOL}`)
        );
        process.exitCode = 2;
      }
    }
