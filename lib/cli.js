...
const meowOptions = {
    ...
    Options:
	  ...
	  --globby-options, --go 
	    ...
	    CLI option to traverse dotfiles i.e Hidden folders files will not be ignored(e.g. stylelint --fix '**/*.css)
	`,
	flags: {
		allowEmptyInput: {
			alias: 'aei',
...
module.exports = async (argv) => {
	...
	// 	const value = cli.flags.globbyOptions;

	// 	try {
	// 		optionsBase.globbyOptions = JSON.parse(value);
	// 	} catch (_) {
	// 		process.stderr.write(
	// 			`Invalid option ${red('"--globby-options"')}. The value ${red(
	// 				`"${value}"`,
	// 			)} is not valid JSON.${EOL}`,
	// 		);
	// 		process.exit(EXIT_CODE_ERROR); // eslint-disable-line no-process-exit
	// 	}
	// }
