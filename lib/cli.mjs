...
/** @import { StyleText } from './utils/createStyleText.mjs' */
...
const helpText = `
  ...
  Input: Files(s), glob(s), or nothing to use stdin.
    ...
    If an input argument is wrapped in quotation marks, it will be expanded 
    as a glob for cross-platform support. "node_modules" are always ignored.
  ...
  Options:
    ...
    --globby-options=<json>
      ...
      Options in JSON format used to match the input globs. Supported options are
      "braceExpansion", "caseSensitiveMatch", "dot", "extglob", "followSymbolicLinks"
      and "ignore".
...
export default async function main(argv) {
	...
	const { color } = cli.flags;
	const styleText = createStyleText(isBoolean(color) ? color : undefined);
	...
	if (shortFlagWithEquals) {
		reportInvalidOption(
			shortFlagWithEquals,
			`${shortFlagWithEquals.slice(0, 2)} ${shortFlagWithEquals.slice(3)}`,
			styleText,
		);
	...
	const booleanFlagWithValue = Object.entries(cli.flags).find(isBooleanFlagWithValue);

	if (booleanFlagWithValue) {
		const [option, value] = booleanFlagWithValue;
	...
	if (isString(globbyOptions)) {
		try {
			options.globbyOptions = await parseGlobbyOptions(globbyOptions, styleText);
		} catch (error) {
	...
	if (options.suppressAll && options.suppressRule) {
		process.stderr.write(
			`${styleText('red', 'Error: The --suppress and --suppress=<rule> options cannot be used together.')}${EOL}`,
		);
		process.exitCode = EXIT_CODE_INVALID_USAGE;
	...
	return standalone(options)
		.then(({ report, code, errored, maxWarningsExceeded }) => {
			...
			if (errored) {
				...
				process.stderr.write(
					`${EOL}${styleText('red', `Max warnings exceeded: `)}${foundWarnings} found. ${styleText(
						'dim',
 ...
 */
function parseGlobbyOptions(value, styleText) {
	const errorMessage = () =>
		`Invalid option ${styleText('red', '"--globby-options"')}.` +
		` The value ${styleText('red', `"${value}"`)} is not valid JSON object.`;
...
/**
 * @param {string} invalid The option as typed, e.g. `--cache=false`.
 * @param {string} suggestion
 * @param {StyleText} styleText
 * @returns {void}
 */
function reportInvalidOption(invalid, suggestion, styleText) {
	process.stderr.write(
		`Invalid option ${styleText('red', `"${invalid}"`)}. Did you mean ${styleText('cyan', `"${suggestion}"`)}?${EOL}`,
	);
	process.exitCode = EXIT_CODE_INVALID_USAGE;
}

/**
 * @param {[string, unknown]} entry An entry of `buildCLI().flags`.
 * @returns {boolean}
 */
function isBooleanFlagWithValue([option, value]) {
	return isString(value) && booleanOptions.has(option);
}
...
function normalizeFlagValue(flag, value) {
	...
	if (flag.type === 'boolean') {
		// An inline value, as in `--cache=false`, stays a string so that `main()` can report it.
		return value;
	}
