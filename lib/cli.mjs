...
import { assert, isPlainObject } from './utils/validateTypes.js';
...
const meowOptions = {
    ...
    Options:
      ...
      --merge-rules
        ...
        Merge rules passed using --rule into the config.
...
export default async function main(argv) {
	...
	if (cli.flags.mergeRules) {
		optionsBase.mergeRules = true;
	}

	if (cli.flags.rule?.length) {
		optionsBase.config = addRules(cli.flags.rule);
	}
...
function addRules(rules) {
	...
	rules.forEach((str) => {
		const [name, value] = str.split(':').map((item) => item.trim());

		if (!name || !value) throw Error("Rule's format is invalid.");

		assert(config);
		assert(config.rules);

		try {
			// TODO @v16 use replaceAll
			config.rules[name.replace(/"/g, '')] = JSON.parse(value);
		} catch (e) {
			throw Error(`Rules' options must be JSON parseable.`);
		}
	});
