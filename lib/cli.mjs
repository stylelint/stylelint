...
/**
 * The parseArgs option for every flag, keyed by the flag's kebab-case name.
 * @type {Record<string, ParseArgsOption>}
 */
const parseArgsOptions = Object.fromEntries(
	Object.entries(flags).map(([name, flag]) => [kebabCase(name), buildParseArgsOption(flag)]),
);
...
export function buildCLI(argv) {
	...
	for (const [name, flag] of Object.entries(flags)) {
		const value = values[kebabCase(name)];
