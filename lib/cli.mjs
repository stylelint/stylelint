...
export default async function main(argv) {
	...
	const resolvedConfig = await resolveConfig(cwd, { configFile, configBasedir });
	const configFormatter = resolvedConfig?.formatter;
	const cliFlagExists = Object.prototype.hasOwnProperty.call(cli.flags, 'formatter');
	const configFormatterExists = configFormatter && formatters[configFormatter];

	if (isString(customFormatter)) {
		formatter = await importCustomFormatter(customFormatter);
	} else if (cliFlagExists && isString(formatterInput)) {
		formatter = /** @type {import('stylelint').FormatterType} */ (formatterInput);
	} else if (configFormatter && !configFormatterExists) {
		formatter = await importCustomFormatter(configFormatter);
	} else if (configFormatter) {
		formatter = /** @type {import('stylelint').FormatterType} */ (configFormatter);
	} else {
