...
const helpText = `
    ...
    Input: Files(s), glob(s), or nothing to use stdin.
      ...
      If an input argument is wrapped in quotation marks, Stylelint will expand
      it as a glob for cross-platform support. "node_modules" are always ignored.
    ...
    Options:
      ...
      --glob-options, --go <json>

        Options in JSON format used to match the input globs. Only "dot" is supported.
...
export default async function main(argv) {
	...
	if (isString(globOptions)) {
		try {
			options.globOptions = await parseGlobOptions(globOptions);
		} catch (error) {
 ...
 */
function parseGlobOptions(value) {
	const errorMessage = () =>
		`Invalid option ${red('"--glob-options"')}.` +
