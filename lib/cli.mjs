...
const helpText = `
    ...
    Options:
      ...
      --report-needless-disables, --rd
        ...
        a lint warning. The process will exit with code ${EXIT_CODE_VIOLATION} if needless disables are found.
      ...
      --report-descriptionless-disables, --rdd
        ...
        exit with code ${EXIT_CODE_VIOLATION} if descriptionless disables are found.

      --max-warnings, --mw <number>

        The number of warnings above which the process will exit with code ${EXIT_CODE_VIOLATION}.
...
export default async function main(argv) {
	...
	return standalone(options)
		.then(({ output, code, errored, maxWarningsExceeded }) => {
			...
			if (errored) {
				process.exitCode = EXIT_CODE_VIOLATION;
