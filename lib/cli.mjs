...
const helpText = `
    ...
    Options:
      ...
      --suppress [<rule-name>]
        Suppress violations. By default the violations are stored in "./${DEFAULT_SUPPRESSION_FILENAME}".
        To adjust this, use "--suppress-location".
        If no rule is specified, it applies to all the currently enabled rules.
        Multiple can be set.
...
export default async function main(argv) {
	...
	if (isBoolean(suppressPrune)) {
		options.suppressPrune = suppressPrune;
	}
	...
	if ((options.suppressAll || options.suppressRule) && options.suppressPrune) {
		process.stderr.write(
			`${red('Error: The --suppress and --suppress-prune options cannot be used together.')}${EOL}`,
		);
		process.exitCode = EXIT_CODE_INVALID_USAGE;
	...
	if (hasStdin && (options.suppressAll || options.suppressRule || options.suppressPrune)) {
		process.stderr.write(
			`${red(
				'Error: The --suppress, --suppress=<rule>, and --suppress-prune options cannot be used with piped-in code.',
			)}${EOL}`,
		);
		process.exitCode = EXIT_CODE_INVALID_USAGE;
