    ...
    Options:
      ...
      Suppressing Violations:
        ...
        --suppress-all
          ...
          Suppress all current violations and write them to "stylelint-suppressions.json".

        --suppress-rule <rule>

          Suppress violations only for the specified rule.

        --suppressions-location <path>

          Specify the location of the suppressions file.

        --prune-suppressions

          Remove entries for violations that no longer exist in the
          codebase from the suppressions file.
...
export default async function main(argv) {
	...
	if (isBoolean(pruneSuppressions)) {
		options.pruneSuppressions = pruneSuppressions;
	}

	if (isBoolean(suppressAll)) {
		options.suppressAll = suppressAll;
	}

	if (isString(suppressionsLocation)) {
		options.suppressionsLocation = suppressionsLocation;
	}

	if (Array.isArray(suppressRule) && suppressRule.length > 0) {
		options.suppressRule = suppressRule;
	} else {
		options.suppressRule = undefined;
	}

	const useStdin = stdin || (!cli.input.length && !process.stdin.isTTY);

	// When `isMultiple: true`, the flag is always an array (never `undefined`).
	// Therefore we detect whether the flag was actually given by checking `suppressRule.length > 0`.
	if (options.suppressAll && options.suppressRule && options.suppressRule.length > 0) {
		process.stderr.write(
			`${red('Error: The --suppress-all option and the --suppress-rule option cannot be used together.')}${EOL}`,
		);
		process.exitCode = EXIT_CODE_INVALID_USAGE;
	...
	if (options.suppressAll && options.pruneSuppressions) {
		process.stderr.write(
			`${red('Error: The --suppress-all option and the --prune-suppressions option cannot be used together.')}${EOL}`,
		);
		process.exitCode = EXIT_CODE_INVALID_USAGE;
	...
	if (options.suppressRule && options.suppressRule.length > 0 && options.pruneSuppressions) {
		process.stderr.write(
			`${red('Error: The --suppress-rule option and the --prune-suppressions option cannot be used together.')}${EOL}`,
		);
		process.exitCode = EXIT_CODE_INVALID_USAGE;
	...
	if (
		...
		process.stderr.write(
			`${red(
				'Error: The --suppress-all, --suppress-rule, and --prune-suppressions options cannot be used with piped-in code.',
			)}${EOL}`,
		);
		process.exitCode = EXIT_CODE_INVALID_USAGE;
