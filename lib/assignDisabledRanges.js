...
module.exports = function (root, result) {
	...
	function endDisabledRange(line, ruleName, strict) {
		...
		const lastRangeForRule = ranges[ranges.length - 1];
	...
	function ruleIsDisabled(ruleName) {
		...
		if (ranges[ranges.length - 1] === undefined) return false;

		if (_.get(ranges[ranges.length - 1], 'end') === undefined) return true;
