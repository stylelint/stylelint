...
const fileRuleCommand = COMMAND_PREFIX + " ";
const ALL_RULES = "all";
...
module.exports = function(
  ...
  const fileRules = (result.stylelint.fileRules = {});
  ...
  function processEnableCommand(comment /*: postcss$comment*/) {
    getCommandRules(enableCommand, comment.text).forEach(ruleToEnable => {
      if (ruleToEnable === ALL_RULES) {
        if (
          _
            .values(disabledRanges)
            .every(ranges => _.isEmpty(ranges) || !!_.last(ranges.end))
        ) {
  ...
  function processFileRuleCommand(comment) {
    const text = comment.text;
    const rules = parseCommandJson(text);
    for (const ruleName of Object.keys(rules)) {
      const ruleValue = rules[ruleName];
      fileRules[ruleName] ||
        (fileRules[ruleName] = Array.isArray(ruleValue)
          ? ruleValue
          : [ruleValue]);
    }
  }
  ...
  function checkComment(comment /*: postcss$comment*/) {
    ...
    } else if (text.indexOf(fileRuleCommand === 0)) {
      processFileRuleCommand(comment);
    }
  }

  function parseCommandJson(fullText) {
    let rules;
    try {
      rules = JSON.parse(`{${fullText.slice(fileRuleCommand.length)}}`);
    } catch (e) {
      rules = {};
    }
    return rules;
  ...
  function getCommandRules(
    ...
    const rules = _
      .compact(fullText.slice(command.length).split(","))
      .map(r => r.trim());
    if (_.isEmpty(rules)) {
      return [ALL_RULES];
    }
