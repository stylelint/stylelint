...
module.exports = function assignDisabledRanges(root, result) {
	...
	root.walkComments((comment) => {
		...
		if (
			!(
				...
				isStylelintCommand(comment, configurationComment) &&
				nextComment &&
		...
		while (
			...
			!isStylelintCommand(current, configurationComment)
		) {
	...
	function checkComment(comment) {
		...
		if (!isStylelintCommand(comment, configurationComment)) {
			return;
		}

		switch (extractStylelintCommand(comment, configurationComment)) {
	...
	function getCommandRules(command, fullText) {
		...
		const fullCommand = getStylelintCommand(command, configurationComment);
