 ...
 */
function createDisabledRange(start, strictStart, end, strictEnd) {
	return {
		start,
		end: end || undefined,
		strictStart,
...
module.exports = function(root /*: Object*/, result /*: Object*/) /*: postcss$result*/ {
	result.stylelint = result.stylelint || /** @type {PostcssResult} */ ({});
	...
	function processEnableCommand(comment) {
		getCommandRules(enableCommand, comment.text).forEach((ruleToEnable) => {
			...
			if (ruleToEnable === ALL_RULES) {
				// @ts-ignore TODO TYPES blocked by https://github.com/stylelint/stylelint/issues/4328
			...
			if (ruleIsDisabled(ALL_RULES) && disabledRanges[ruleToEnable] === undefined) {
				...
				if (!disabledRanges[ruleToEnable]) {
					disabledRanges[ruleToEnable] = disabledRanges.all.map(({ start, end }) =>
						createDisabledRange(start, false, end, false),
					);
				} else {
					disabledRanges[ruleToEnable].push(
						/** @type {DisabledRange} */ (_.clone(_.last(disabledRanges[ALL_RULES]))),
					);
				}
	...
	function startDisabledRange(line, ruleName, strict) {
		const rangeObj = createDisabledRange(line, strict);
