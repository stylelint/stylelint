...
export default async function lintPostcssResult(stylelintOptions, postcssResult, config) {
	...
	for (const ruleName of ruleNames) {
		 ...
		 */
		async function timedRuleFn(postcssRoot) {
			return await timing.time(ruleName, async () => ruleFn(postcssRoot, postcssResult))();
		}

		performRules.push(Promise.all(postcssRoots.map(timedRuleFn)));
	}
