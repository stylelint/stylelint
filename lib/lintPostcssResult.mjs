...
export default async function lintPostcssResult(stylelintOptions, postcssResult, config) {
	...
	const isFileFixCompatible = isFixingAllowed(postcssResult, stylelintOptions);
	...
	for (const ruleName of ruleNames) {
		...
		const disableFix = secondaryOptions?.disableFix || false;
		const fileFix = !disableFix && config.fix && isFileFixCompatible;
		// until #2643 is resolved
		const ruleFix =
			!stylelintOptions.safeDisables || !postcssResult.stylelint.disabledRanges[ruleName];
 ...
 * @returns {boolean}
 */
function isFixingAllowed({ stylelint }, { safeDisables }) {
	return !safeDisables || !stylelint.disabledRanges[RULE_NAME_ALL]?.length;
}
