...
import resolveConfigForFile from './utils/resolveConfigForFile.mjs';

const IS_TEST = process.env.NODE_ENV === 'test';
const STOP_DIR = IS_TEST ? process.cwd() : undefined;

/** @import {CosmiconfigResult, InternalApi, Config} from 'stylelint' */
 ...
 * @param {CosmiconfigResult | undefined} [cachedConfig] - For applying overrides.
 * @param {Object<string, Config> | undefined} [fileConfigMap] - Throws an error if a config is not found.
...
export default async function getConfigForFile(
	...
	if (cachedConfig?.config && filePath && fileConfigMap) {
		const configForFile = resolveConfigForFile(filePath, cachedConfig.config, fileConfigMap);
		...
		if (configForFile) {
			...
			const fileConfig = {
				config: {
					...configForFile.other,
					rules: configForFile.rules,
				},
				filepath: stylelint._options.configFile || '',
			};
