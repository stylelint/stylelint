...
/** @import {Node as PostcssNode, Comment as PostcssComment, Document as PostcssDocument, Root as PostcssRoot, Position as PostcssPosition} from 'postcss' */
/** @import {DisabledRange, DisabledRangeObject, PostcssResult} from 'stylelint' */
/** @typedef {{start?: PostcssPosition, end?: PostcssPosition}} Source */
 ...
 * @returns {boolean}
 */
function containsComments(string) {
...
export default function assignDisabledRanges(root, result) {
	...
	root.walk((node) => {
		if (isComment(node)) {
			...
			let lastLine = (node.source && node.source.end && node.source.end.line) || 0;
		...
		if (isRule(node)) {
			...
			if (selector && containsComments(selector)) {
				checkCommentContainingNode(node, selector, offset);
			}

			offset += selector?.length ?? node.selector.length;
			const between = node.raws?.between;

			if (between && containsComments(between)) {
				checkCommentContainingNode(node, between, offset);
			}
		}

		if (isAtRule(node)) {
			let offset = node.name.length + 1;
			const afterName = node.raws?.afterName;

			if (afterName && containsComments(afterName)) {
				checkCommentContainingNode(node, afterName, offset);
			}

			offset += afterName?.length ?? 0;
			const params = node.raws?.params?.raw;

			if (params && containsComments(params)) {
				checkCommentContainingNode(node, params, offset);
			}
		...
		if (isDeclaration(node)) {
			...
			if (value && containsComments(value)) {
				checkCommentContainingNode(node, value, offset);
			}
		}
	});
	 ...
	 * @param {number} offset
	 */
	function checkCommentContainingNode(node, part, offset) {
		tokenize({ css: part })
			.filter((token) => {
				return isTokenComment(token);
			})
			.forEach((token) => {
