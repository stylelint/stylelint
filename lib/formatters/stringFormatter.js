...
function formatter(messages, source) {
	...
	const calculateWidths = function(columns) {
		_.forOwn(columns, (value, key) => {
			...
			columnWidths[k] = Math.max(columnWidths[k], stringWidth(normalisedValue));
		});
	...
	const cleanedMessages = orderedMessages.map((message) => {
		// TODO TYPES looks like StylelintWarning is misleading type here
		// it does not have node property
		...
		const row = [
			...
			// TODO TYPES maybe add `success` to levelColors?
			symbols[severity] && levelColors[severity]
...
module.exports = function(results) {
	...
	output = results.reduce((output, result) => {
		// TODO TYPES did not found out from short look where stylelintType defines
