...
describe('verboseFormatter', () => {
	...
	suite('fixed problems', () => {
		it('outputs a list of rules that have been fixed', () => {
			const results = [
				{
					source: 'path/to/file.css',
					errored: false,
			...
			expect(output).toBe(stripIndent`
				path/to/file.css
				  2:3  ‼  Empty block  block-no-empty
		...
		test('multiple files with different rules', () => {
			const results = [
				{
	...
	it('outputs color depending on the `color` option', () => {
		const results = [
			{
				source: 'path/to/file.css',
				errored: false,
		...
		expect(verboseFormatter(results, { ruleMetadata: {}, color: true })).toContain('\u001B[');
		expect(verboseFormatter(results, { ruleMetadata: {}, color: false })).not.toContain('\u001B[');
	});
});
