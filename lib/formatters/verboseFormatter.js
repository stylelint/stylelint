...
module.exports = function (results, returnValue) {
	...
	for (const [severityLevel, warningList] of Object.entries(warningsBySeverity)) {
		...
		for (const [rule, list] of Object.entries(warningsByRule)) {
			output += dim(`  ${ruleLink(rule)}: ${list.length}\n`);
		}
	}
...
function fileLink(source) {
	...
	return terminalLink(source, `file://${source}`, { fallback: false });
}
...
function ruleLink(rule) {
	...
	const url = `https://stylelint.io/user-guide/rules/list/${rule}`;

	return terminalLink(rule, url, { fallback: false });
}
