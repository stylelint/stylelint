...
import calcMaybeFixableCounts from './calcMaybeFixableCounts.mjs';
...
export default function stringFormatter(results, returnValue) {
	...
	output = results.reduce((accum, result) => {
		...
		for (const warning of result.warnings) {
			...
			calcMaybeFixableCounts(
				warning.severity,
				returnValue.ruleMetadata?.[warning.rule]?.fixable ?? false,
				fixableCounts,
			);
		}
	...
	if (output !== '') {
		...
		if (total > 0) {
			...
			output += `\n${symbols.error} ${total} ${pluralize('problem', total)} (${error}, ${warning})`;
		}
