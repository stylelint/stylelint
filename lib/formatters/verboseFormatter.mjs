...
function getFixedRules(results) {
	...
	for (const { _postcssResult } of results) {
		...
		for (const ruleName in fixersData) {
			if (Object.hasOwn(fixersData, ruleName)) {
				const filteredArray = fixersData[ruleName]?.filter(({ fixed }) => fixed);
				const count = filteredArray?.length;

				if (count) rules[ruleName] = rules[ruleName] ? rules[ruleName] + count : count;
			}
		}
	}
