...
- `configOverrides` now work with `extends` ([#2295](https://github.com/stylelint/stylelint/pull/2295)).
...
- `"always"` and `"never"` option to `rule-properties-order` `emptyLineBefore` when using property groups
...
- bug in `rule-properties-order` empty line detection when the two newlines were separated
  by some other whitespace.
- option parsing bug that caused problems when using the `"alphabetical"` primary option
